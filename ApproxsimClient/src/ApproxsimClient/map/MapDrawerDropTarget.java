...
class MapDrawerDropTarget implements DropTargetListener {
    ...
    /**
     * Called when the drag operation has terminated with a drop on the operable part of the drop site for the DropTarget registered with
     * this listener.
     */
        ...
        try {
            if (dtde.isDataFlavorSupported(ApproxsimObject.APPROXSIM_OBJECT_FLAVOR)) {
                    ...
                    if (sc.getType().canSubstitute("Order")) {
                        ...
                        if (!outlinedElements.isEmpty()) {
                            ...
                            if (outlinedElements.size() == 1) {
                                MapElementAdapter meAdapter = (MapElementAdapter) outlinedElements
                                        .elements().nextElement();
                                addActivityToMilitaryUnit(sc,
                                                          meAdapter.getObject());
                            ...
                            // open a menu over military units at the location of the dropped activity
                                ...
                                menu.add(getMenuForMilitaryUnits(outlinedElements,
                                                                 sc));
                                // show the menu
                                menu.show(drawer, (int) pt.getX(),
                                          (int) pt.getY());
                            }
                            complete = true;
                        }
                ...
                dtde.dropComplete(complete);
                    ...
                    .isDataFlavorSupported(HierarchyImportSet.HIERARCHYIMPORTSET_FLAVOR)) {
                dtde.acceptDrop(DnDConstants.ACTION_LINK);
                dropAccepted = true;
                // get the dropped object
                Object obj = dtde
                        .getTransferable()
                        .getTransferData(HierarchyImportSet.HIERARCHYIMPORTSET_FLAVOR);
                // Apple's dnd implementation sucks... We must call the getTransferData method for the string
                // flavor in order to get a valid callback.
                dtde.getTransferable().getTransferData(DataFlavor.stringFlavor);
                HierarchyImportSet his = (HierarchyImportSet) obj;
                // Transfer imported objects to simulation at position lat, lon
                dtde.dropComplete(his.transferToSimulation(client
                        .getRootObject(), lat, lon));
                drawer.update();
            } else {
                dtde.rejectDrop();
            }
    ...
    /**
     * This method opens a dialog when the dropped element already exists on the map. The user can choose to replace the existing element
     * with the dropped element or not.
     * 
        ...
        for (Enumeration e = militaryUnits.elements(); e.hasMoreElements();) {
            ...
            item.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent event) {
                    for (Enumeration en = mUnits.elements(); en
                            .hasMoreElements();) {
                        ((MapElementAdapter) en.nextElement())
                                .setOutlined(false);
                    }
