...
public class BoundingBox {
     ...
     */
    public BoundingBox(double westLon, double southLat, double eastLon, double northLat) {
     ...
     */
    public BoundingBox(double xmin, double ymin, double xmax, double ymax, Projection proj) {
        this.xmin = xmin;
        this.xmax = xmax;
        this.ymin = ymin;
        this.ymax = ymax;
        ...
        this.eastLon = (westLon > eastNorth[0])? eastNorth[0] + 360 : eastNorth[0];
        this.southLat = westSouth[1];
        this.northLat = (southLat > eastNorth[1])? eastNorth[1] + 180 : eastNorth[1];
    }
    ...
    public boolean intersects(Projection proj, BoundingBox bbox) {
        ...
        double[] bboxXYMin = proj.projToXY(bbox.getWestLon(), bbox.getSouthLat());
        double[] bboxXYMax = proj.projToXY(bbox.getEastLon(), bbox.getNorthLat());
    ...
    {
        this.westLon  = this.getWestLon() < other.getWestLon() ? this.getWestLon() : other.getWestLon();
        this.southLat = this.getSouthLat() < other.getSouthLat() ? this.getSouthLat() : other.getSouthLat();

        this.eastLon  = this.getEastLon() > other.getEastLon() ? this.getEastLon() : other.getEastLon();
        this.northLat = this.getNorthLat() > other.getNorthLat() ? this.getNorthLat() : other.getNorthLat();
    }
     ...
     */
    public static BoundingBox combine(BoundingBox box1, BoundingBox box2, Projection proj)
    {
        box1.setProjection(proj);
        box2.setProjection(proj);

        double xmin = box1.getXmin() < box2.getXmin() ? box1.getXmin() : box2.getXmin();
        double ymin = box1.getYmin() < box2.getYmin() ? box1.getYmin() : box2.getYmin();

        double xmax  = box1.getXmax() > box2.getXmax() ? box1.getXmax() : box2.getXmax();
        double ymax = box1.getYmax() > box2.getYmax() ? box1.getYmax() : box2.getYmax();
