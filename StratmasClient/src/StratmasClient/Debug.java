...
/**
 * Debug is a class used for debug operations of different kinds. The point of the class is to allow easy on and off switching of, for
...
public class Debug {
    ...
    /**
     * Returns an adaption of stderr that allows printing from classes whose package name matches the provided regex.
     * 
     * @param pattern the pattern to match the package name against if pattern.equals("") or pattern.equals(".*") all messages are let
     *            through.
     */
        ...
        if (pattern.equals(".*") || pattern.equals("")) {
            ...
            return new PrintStream(new OutputStream() {
                ...
                public boolean doWrite() {
                    ...
                    StackTraceElement[] frames = new Throwable()
                            .getStackTrace();

                    for (int i = 0; i < frames.length; i++) {
                        // Skip frames created in this class.
                        if (!frames[i].getClassName()
                                .equals(getClass().getName())) {
                            String fields[] = frames[i].getClassName()
                                    .split("\\.");
                            if (fields.length > 1
                                    && fields[0].equals(Debug.class
                                            .getPackage().getName())) {
    ...
    public static JPanel createDebugPanel() {
        ...
        for (Iterator<Action> it = getDebugActions().iterator(); it.hasNext(); panel
    ...
    /**
     * Adds a debug component that will be visible in debug frames. NOTE: it is much preferable to, if possible, define an action instead.
     * 
    ...
    /**
     * Returns the StackTraceElement of the calling method of the calling method, or null if no such method.
     */
    ...
    /**
     * Returns the StackTraceElement of the first function that is defined outside the calling class, or null if none.
     */
