...
public class HierarchyImportSet implements Transferable {
    ...
    /**
     * Accessor for the root of the objects that will be / are imported to the simulation.
     * 
    ...
    /**
     * Transfers selected objects in the hierarcy to the simulation at the specified position.
     * 
     * @param root the root where the simulation to transfer objects to lives.
        ...
        for (; en.hasMoreElements();) {
            ...
            if (hoa.isSelected() && !hoa.isUsed()) {
                ...
                if (unitList.getChild(toAdd.getIdentifier()) == null) {
                    ...
                    Debug.err.println("Object with identifier "
                            + toAdd.getIdentifier() + " already exists.");
                    return false;
                }
            }
            for (Enumeration en2 = hoa.children(); en2.hasMoreElements();) {
                transferToSimulation((HierarchyObjectAdapter) en2.nextElement(),
                                     lat, lon);
            }
        }
     ...
     * @return true if the transfer was succesful - false otherwise.
     */
    private void transferToSimulation(HierarchyObjectAdapter obj, double lat,
            double lon) {
        ...
        for (Enumeration en = obj.children(); en.hasMoreElements();) {
            transferToSimulation((HierarchyObjectAdapter) en.nextElement(),
                                 lat, lon);
        }
    }
    ...
    private void setLocation(StratmasObject obj, double lat, double lon) {
        ...
        if (location != null) {
            if (location instanceof Shape) {
                ...
                System.err
                        .println("Location not a Shape. Cannot set location for type"
    ...
    public synchronized DataFlavor[] getTransferDataFlavors() {
        return new DataFlavor[] { HIERARCHYIMPORTSET_FLAVOR,
     ...
     * @return true if the specified flavor is supported, false otherwise.
     */
