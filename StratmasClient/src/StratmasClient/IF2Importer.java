...
public class IF2Importer {
    ...
    /**
     * Imports units from an IF2 project. Sets the units' identifier, symbolIDCode and subunits attributes. Asks the user for an affiliation
    ...
    public ApproxsimObject importFromFile(String fileName) {
        ...
        try {
            ...
            if (!docElem.getTagName().equals("IF2Project")) {
                setErrorMessage("Root element has tag '"
                        + docElem.getNodeName() + "'. Should be IF2Project'");
                return null;
            }
    ...
    /**
     * Imports units from an IF2 project. Sets the units' identifier, symbolIDCode and subunits attributes.
     * 
     * @param fileName The name of the file to import from.
    ...
    /**
     * Helper for creating military units with a specified id, symbol id code and set of subunits, where the rest of the attributes are
     * given their default values except for the affiliation faction for which the user is asked..
     * 
    ...
    public ApproxsimObject createDefaultImportedUnit(String id, String symCode,
        ...
        try {
            ((SymbolIDCode) unit.getChild("symbolIDCode"))
                    .valueFromString(symCode, null);
