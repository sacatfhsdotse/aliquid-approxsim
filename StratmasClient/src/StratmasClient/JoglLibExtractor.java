...
public class JoglLibExtractor {
    /**
     * Extracts the jogl dynamic libraries, puts them in a temporary directory.
     */
    protected synchronized static boolean initializeJoglLibs(String[] parameters) {
        if (parameters.length != 0 && parameters[0].equals("-noJoglResolve")) {
            return true;
    ...
    protected synchronized static boolean doInitialize() {
        ...
        if (extractToDir(tempdir, true)) {
            ...
            for (int i = 0; i < libNames.length; i++) {
                System.err.println("Registering with JogAmp: "
                        + libNames[i].replaceAll("\\..*$", "")
                                .replaceAll("^lib", ""));
                JNILibLoaderBase.addLoaded(libNames[i].replaceAll("\\..*$", "")
                        .replaceAll("^lib", ""));
            }
            return true;
        } else {
    ...
    protected synchronized static boolean doReExec(String[] parameters) {
        ...
        if (extractToDir(tempdir, false)) {
            ...
            String jarLocation = JoglLibExtractor.class
                    .getResource("JoglLibExtractor.class").getPath();
            try {
                jarLocation = jarLocation
                        .substring(jarLocation.indexOf(':') + 1,
                                   jarLocation.indexOf('!'));
            } catch (IndexOutOfBoundsException e) {
                System.err
                        .println("Unable to construct command line for automatic JOGL library loading under "
                                + System.getProperty("os.name"));
                System.exit(1);
            }
            ...
            System.err
                    .println("Unable to load required libraries. Please try starting the program as follows:\n"
                            + envp[0] + buf.toString());

            javax.swing.JOptionPane
                    .showMessageDialog(null,
                                       "Unable to load required libraries. Please try starting the program as follows:\n"
                                               + envp[0] + buf.toString(),
    ...
    protected static boolean extractToDir(String resource, File file) {
        try {
            ...
            System.err.println("Extracting: " + resource + " to "
                    + file.getPath());
    ...
    protected static String[] joglOsLibNames() {
        ...
        if (os.equals("Linux") || os.equals("SunOS")) {
            return new String[] { "libjogl_desktop.so", "libgluegen-rt.so",
                    "libnativewindow_awt.so", "libnativewindow_x11.so" };
        } else if (os.equals("Mac OS X")) {
            return new String[] { "libjogl_desktop.jnilib",
                    "libgluegen-rt.jnilib", "libnativewindow_awt.jnilib",
                    "libnativewindow_macosx.jnilib" };
        } else if (os.matches("Windows.*")) {
            return new String[] { "jogl_desktop.dll", "gluegen-rt.dll",
                    "nativewindow_awt.dll", "nativewindow_win32.dll" };
        } else {
            return null;
        }
    }
    ...
    /**
     * Loads the jogl libraries. Acknowledgement: Taken directly from: com.sun.opengl.impl.NativeLibLoader:load()
     * 
                ...
                .doPrivileged(new java.security.PrivilegedAction() {
                    public Object run() {
                        ...
                        boolean isLinux = System.getProperty("os.name")
                                .equals("Linux");
                        boolean isAmd64 = System.getProperty("os.arch")
                                .equals("amd64");
                        if (!isOSX) {
                            try {
