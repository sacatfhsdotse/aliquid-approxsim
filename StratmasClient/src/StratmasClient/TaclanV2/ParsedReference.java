...
/**
 * An object representing a reference to some declared object of the Taclan language. As such it contains artefacts of the language, it also
 * contains the non lexical type checks.
 * 
 * @version 1, 09/28/04
 * @author Daniel Ahlin
 */
...
public class ParsedReference extends ParsedPrimitive {
    ...
    /**
     * Binds any reference to its intended target. For a reference that means returning a Vector containing itself, if not previously bound.
     */
    ...
    /**
     * Tries to bind this ParsedReference by traversing the given ParsedDeclaration.
     * 
    ...
    public void bind(ParsedDeclaration declaration) throws SemanticException {
        ...
        if (this.getTail() != null) {
            ParsedDeclaration target = declaration.getDeclarations()
                    .getDeclaration(this.getTail().getHead());
            if (target != null) {
    ...
    /**
     * Returns true if this reference has a tail (i. e. if this reference has more than one component.)
     */
