...
public class UnitImportFilter {
        ...
        for (String line = lineReader.readLine(); line != null; line = lineReader
            ...
            if (fields.length != headers.length) {
                throw new ParseException("Wrong number of fields "
                        + fields.length + " (should be) " + headers.length,
    ...
    /**
     * Classifies a header as either input, output or non interesting. ret >= 0: input and position in input ret == -1 not interesting. ret
     * == -2 output
     * 
     * @param header the header to classify.
     */
    private int classifyColumn(String header) {
        if (header == null || header == "" || header.length() == 0
                || header.matches("\\s+")) {
            return 0;
    ...
    /**
     * Sets values of the provided unit to match its symbolIDCode. If the unit lacks a symbolIDCode nothing is done.
     * 
    ...
    public Vector apply(ApproxsimObject unit) {
        ...
        for (int i = 0; i < inTable.length; i++) {
            ...
            for (int j = 0; j < matchFields.length; j++) {
                ...
                if (!(inTable[i][j] == null || inTable[i][j]
                        .equals(matchFields[j]))) {
        ...
        for (int j = 0; j < outTableHeaders.length; j++) {
            ...
            if (child == null) {
                Declaration declaration = unit.getType()
                        .getSubElement(outTableHeaders[j]);
                if (declaration == null) {
            ...
            if (child instanceof ApproxsimSimple) {
                ...
                if (requestedType == null
                        || !requestedType
                                .canSubstitute(unit.getType()
                                        .getSubElement(child.getIdentifier())
                                        .getType())) {
