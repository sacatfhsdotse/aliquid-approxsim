...
public class XMLImporter {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.err.println(e.getMessage());
            for (Enumeration<SAXException> en = e.getExceptions().elements(); en
                    .hasMoreElements();) {
                System.err.println("==============================");
...
class SOPlaceHolder {
     ...
     * 
     * @param identifier The idenfifier of the object this placeholder refers to.
...
class SAXDocumentHandler extends DefaultHandler implements EntityResolver2 {
    /**
     * Non-fatal exceptions that occurs during parsing are collected in this vector.
     */
    Vector<SAXException> exceptions = new Vector<SAXException>();

    /**
     * Flag marking whether the current element is imported from an external entity or not.
     */
    private boolean imported = false;

    /**
     * This flag indicates if we should care about characters in the characters() method.
     */
    ...
    /**
     * The array in which characters are stored between consecutive calls to the characters method that belongs to the same simple type.
     */
    private char[] chars;

    /**
     * The Vector in which identifiers are stored while parsing a Reference.
     */
    ...
    /**
     * A stack on which we push the type of the element currently beeing created.
     */
     ...
     * <p>
     * The Parser will call this method to report each chunk of character data. SAX parsers may return all contiguous character data in a
     * single chunk, or they may split it into several chunks; however, all of the characters in any single event must come from the same
     ...
     * </p>
     * <p>
     * Individual characters may consist of more than one Java <code>char</code> value. There are two important cases where this happens,
     * because characters can't be represented in just sixteen bits. In one case, characters are represented in a <em>Surrogate Pair</em>,
     * using two special Unicode values. Such characters are in the so-called "Astral Planes", with a code point above U+FFFF. A second case
     * involves composite characters, such as a base character combining with one or more accent characters.
     * </p>
     * <p>
     * Your code should not assume that algorithms using <code>char</code>-at-a-time idioms will be working in character units; in some
     * cases they will split characters. This is relevant wherever XML permits arbitrary characters, such as attribute values, processing
     * instruction data, and comments as well as in data reported from this method. It's also generally relevant whenever Java code
     * manipulates internationalized text; the issue isn't unique to XML.
     * </p>
     * <p>
     * Note that some parsers will report whitespace in element content using the {@link #ignorableWhitespace ignorableWhitespace} method
     * rather than this one (validating parsers <em>must</em> do so).
     * </p>
     * 
    ...
    public void characters(char[] ch, int start, int length)
        ...
        if (readChars == true) {
            ...
            if (currentLength + length > allocatedLength) {
                ...
                chars = tmp;
                Debug.err
                        .println("SAX Document handler reallocating char array.");
            }
     ...
     * <p>
     * The Parser will invoke this method at the beginning of every element in the XML document; there will be a corresponding
     * {@link #endElement endElement} event for every startElement event (even when the element is empty). All of the element's content will
     ...
     * </ol>
     * <p>
     * Any or all of these may be provided, depending on the values of the <var>http://xml.org/sax/features/namespaces</var> and the
     * <var>http://xml.org/sax/features/namespace-prefixes</var> properties:
     * </p>
     * <ul>
     * <li>the Namespace URI and local name are required when the namespaces property is <var>true</var> (the default), and are optional
     * when the namespaces property is <var>false</var> (if one is specified, both must be);</li>
     * <li>the qualified name is required when the namespace-prefixes property is <var>true</var>, and is optional when the
     * namespace-prefixes property is <var>false</var> (the default).</li>
     * </ul>
     * <p>
     * Note that the attribute list provided will contain only attributes with explicit values (specified or defaulted): #IMPLIED attributes
     * will be omitted. The attribute list will contain attributes used for Namespace declarations (xmlns* attributes) only if the
     * <code>http://xml.org/sax/features/namespace-prefixes</code> property is true (it is false by default, and support for a true value is
     * optional).
     * </p>
     * <p>
     * Like {@link #characters characters()}, attribute values may have characters that need more than one <code>char</code> value.
     * </p>
     * 
     ...
     * @param atts the attributes attached to the element. If there are no attributes, it shall be an empty Attributes object. The value of
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        if (type == null) {
            ...
            if (type.canSubstitute("Reference",
                ...
                currentPlaceHolder = new SOPlaceHolder(currentIdentifier, type,
                        currentPlaceHolder);
            }
     ...
     * <p>
     * The SAX parser will invoke this method at the end of every element in the XML document; there will be a corresponding
     * {@link #startElement startElement} event for every endElement event (even when the element is empty).
     * </p>
     * <p>
        ...
        if (readChars == true) {
            ...
            if (type.getName().equals("Point")) {
                ...
                if (localName.equals("lat")) {
                    ((Point) currentObject)
                            .setLat(Double.parseDouble(new String(chars, 0,
                                    currentLength)), null);
                } else if (localName.equals("lon")) {
                    ((Point) currentObject)
                            .setLon(Double.parseDouble(new String(chars, 0,
                                    currentLength)), null);
                }
            ...
            if (type.canSubstitute("Reference",
                ...
                try {
                    ...
                    String msg = "";
                    if (locator != null) {
                        msg = "Line: " + locator.getLineNumber() + ", column: "
     ...
     *            URI, unless it is null because the XMLReader was given an InputSource without one.
     * @return An InputSource object describing the new external subset to be used by the parser, or null to indicate that no external
     *         subset is provided.
     */
    ...
    /**
     * Resolves references to ESRI .shp files and schemas possibly located in a jar-file. If the referenced entity is a file ending with
     * '.shp' a stream to the ESRI file that produces XML from the binary data is opened and returned to the application. For both .shp
     * files and .xml files the import flag is set to true in order to allow the application to collect information about which elements
     * that were imported from external sources.
     * 
     * @param name Identifies the external entity being resolved. Either "[dtd]" for the external subset, or a name starting with "%" to
     *            indicate a parameter entity, or else the name of a general entity. This is never null when invoked by a SAX2 parser.
     ...
     * @param baseURI The URI with respect to which relative systemIDs are interpreted. This is always an absolute URI, unless it is null
     *            (likely because the XMLReader was given an InputSource without one). This URI is defined by the XML specification to be
     *            the one associated with the "&lt;" starting the relevant declaration.
     * @param systemId The system identifier of the external entity being referenced; either a relative or absolute URI. This is never null
     *            when invoked by a SAX2 parser; only declared entities, and any external subset, are resolved by such parsers.
     * @return An InputSource object describing the new input source to be used by the parser. Returning null directs the parser to resolve
     *         the system ID against the base URI and open a connection to resulting URI.
     * @exception SAXException Any SAX exception, possibly wrapping another exception.
     * @exception IOException Probably indicating a failure to create a new InputStream or Reader, or an illegal URL.
     */
    ...
    /**
     * Tries to locate a file from the given systemId and baseUri. First checks if the systemId is an absolute path and second if it is
     * relative to the folder containing the file pointed to by baseURI.
     * 
    ...
    private URL findFile(String baseURI, String systemId) {
        ...
        try {
            ...
            if (file.isAbsolute() && file.exists()) {
                ...
                File baseFile = new File(URLDecoder.decode(baseURL.getPath(),
                                                           enc))
                        .getParentFile();
                String absPath = baseFile.getPath()
        ...
        } catch (MalformedURLException e) {} catch (UnsupportedEncodingException e) {}
        return ret;
    }

    /**
     * Handles warnings from the parser. Stores the given exception and lets the parser continue unless the total number of exceptions is to
     * large.
     * 
     * @param exception The exception found by the parser.
     * @throws SAXException Throws the last exception when to many exceptions has occurred.
     */
    ...
    /**
     * Handles errors from the parser. Stores the given exception and lets the parser continue unless the total number of exceptions is to
     * large.
     * 
     * @param exception The exception found by the parser.
     * @throws SAXException Throws the last exception when to many exceptions has occurred.
    ...
    /**
     * Adds line and column number information to the beginning of the message of the given exception.
     * 
     * @param e The exception to add line and column number information to.
     * @return A new exception with line and column number information added to the message.
     */
    private SAXParseException addLineAndColumnInfoToMessage(SAXParseException e) {
        String msg = "Line: " + e.getLineNumber() + ", column: "
                + e.getColumnNumber() + ": ";
        return new SAXParseException(msg + e.getMessage(), e.getPublicId(),
...
/**
 * Exception thrown when SOPlaceHolder gets null from the vector constructor.
 * 
 * @version 1, $Date: 2006/09/18 09:45:25 $
 * @author Per Alexius
 */
