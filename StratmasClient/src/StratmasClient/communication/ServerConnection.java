...
public class ServerConnection implements Runnable {
    ...
    public ServerConnection(Client client, XMLHandler xh, String host,
            int port, long id) {
        mHost = host;
        mPort = port;
        mXMLHandler = xh;
    ...
    /**
     * Enqueues a message in the priorityqueue. The message will be sent by the ServerConnection thread when it reaches the front of the
     * PriorityQueue. This method is non-blocking.
     * 
    ...
    /**
     * Enqueues a message in the priorityqueue. The message will be sent by the ServerConnection thread when it reaches the front of the
     * PriorityQueue. This method is blocking and does not return until the XMLHandler has signaled that it has handled the contents of the
     * response to the sent message. !!! Notice that a deadlock situation will occur if the thread calling this method owns an object needed
     * to complete the sending, receiving and handling of the message and its response, for example the Client object.
     * 
     * @param msg The message to be sent.
     */
     ...
     * 
     * @return True if theconnection to the server is open, false otherwise.
     */
    ...
    /**
     * Closes this connection and eventually terminates the thread running it.
     */
     ...
     * 
     * @param timeout number of milliseconds to wait before ruthlessly killing the connection.
     */
    ...
    /**
     * Kills this connection and the thread running it. Doesn't care about the consequences.
     */
    public void kill() {
        mQuitRuthless = true;
        disconnect();
     ...
     * 
     * @param flag true if the connection should be regarded as connected, false otherwise.
     */
...
//          tsdf = TimeSliderDebugFrame.openTimeSliderDebugFrame();
        ...
        try {
            ...
            sendErrorMessage("general", "Connection to " + mHost + ":" + mPort
                    + " rejected", "ConnectResponseMessage");
        } catch (UnknownHostException e) {
            sendErrorMessage("general", "Unknown host: " + mHost,
                             "ConnectResponseMessage");
        } catch (java.net.SocketException e) {
            sendErrorMessage("general",
                             "Connection closed. Is the server really running?",
                             "Unknown");
