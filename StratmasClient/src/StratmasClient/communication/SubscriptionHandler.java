...
public class SubscriptionHandler implements Runnable {
    ...
    /**
     * The time to wait after receiving a subscription before sending a subscription message.
     */
    ...
    /**
     * Creates a new SubscriptionHandler and connects it to the provided ServerConnection.
     * 
    ...
    /**
     * Connects this SubscriptionHandler to the specified ServerConnection.
     * 
     ...
     * @param sub The Subscription to register.
     */
    public void blockingRegSubscription(Subscription sub)
        ...
        while (!mQuit) {
            if (msg.isEmpty()) {
                ...
                while (System.currentTimeMillis() - firstDequeueTime < sDelayms
                        && msg.size() < sMaxNumSubsPerMsg && !mQuit) {
                    sub = (Subscription) mQueue.dequeue();
                    if (sub != null) {
                        msg.addSubscription(sub);
                    }
                    SubscriptionCounter.updateNrOfMessInSendingQueue2(msg
                            .size());
                }
                if (mQuit) {
                    break;
                ...
                if (mSC != null) {
                    ...
                    System.err
                            .println("No ServerConnection in SubscriptionHandler!");
                    mQuit = true;
                }
            }
            SubscriptionCounter.updateNrOfMessInSendingQueue2(msg.size());
    ...
    public void handleSubscribedData(org.w3c.dom.Element n, Timestamp t) {
        ...
        if (sub != null) {
            ...
            Debug.err.println("Can't find subscription of type '"
                    + n.getAttribute("xsi:type") + "' with id " + id);
        }
    }
}
