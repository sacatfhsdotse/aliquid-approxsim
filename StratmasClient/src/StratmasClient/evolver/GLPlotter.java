...
public class GLPlotter extends JPanel implements GLEventListener, MatrixEventListener
{
    ...
    ColorMapper colorMapper = new ColorMapper()
        {
            public double[] map(double d)
            {
                ...
                return new double[] 
                    {// Blue
                     mappedValue > Math.PI ? 
                     Math.cos((mappedValue - Math.PI)/1.5) : 0,
                     
                     // Green
                     mappedValue > 0.5 * 
                     Math.PI && mappedValue < 1.5 * Math.PI ? 
                     Math.cos((mappedValue - 0.5 * Math.PI)/1.5) : 0,
                     
                     // Red
                     mappedValue < Math.PI ? 
                     Math.cos((mappedValue)/1.5) : 0.0d,
    ...
    public void setEvolver(Evolver evolver)
    {
        if (evolver != null) {
            ...
            setYParameter((Parameter) evolver.getParameters().get(1 % nParameters));
    ...
    public void setMatrix(EvaluationsMatrix matrix)
    {
        ...
        setParameterSlice(new double[getMatrix().getColumnMap().getParameters().size()]);
    ...
    /**
     * Sets the parameter slice used to get the value for parameters
     * that are not used for axes parameters. Note that the slice is
     * expected to be
     * getMatrix().getColumnMap().getParameters().size() long.
     *
    ...
    /**
     * Returns the parameter slice used to get the value for
     * parameters that are not used for axes parameters. Note that the
     * slice is expected to be
     * getMatrix().getColumnMap().getParameters().size() long.
     */
    ...
    /**
     * Returns an array with the smallest and largest value for the
     * parameter.
     *
    ...
    /**
     * Updates span if new span is outside current span, if so
     * increase span by 20 % over and under the new value. If the span
     * is smaller than 20% over and under, decreases until exactly 20%
     * over and under new value.
     *
    ...
    /**
     * Fills in a two-dimensional grid of specified resolution within
     * [getXMin(), getXMax()] and [getYMin(), getYMax()].
     *
     * @param xTiles number of grid cells along the x-axis.
     * @param yTiles number of grid cells along the z-axis.
     * @param grid the array to fill in, if (xTiles + 1) * (yTiles + 1) * 4 >
     * grid.length the behaviour is undefined. The grid will be filled as follows:
     * Point n: grid[n + 0] x component
     *          grid[n + 1] y component
     *          grid[n + 2] z component
     *          grid[n + 3] color component
     *
     * The points will be sorted on the x component and the z
    ...
    protected double[] updateGrid(int xTiles, int yTiles, double[] grid)
    {
        ...
        for (int i = 0; i < xTiles + 1; i++) {
            ...
            for (int j = 0; j < yTiles + 1; j++) {
                ...
                values[yIndex] = y;
                values[xIndex] = x;
                double[] point = Interpolator.interpolate(samples,
                                                          iIndices,
                                                          values);
     ...
     * device associated with the GLAutoDrawable has changed.
     * 
     * @param drawable 
     * @param modeChanged
     * @param deviceChanged
    ...
    public void recompileSamples(GLAutoDrawable drawable)
    {
        ...
        topDisplayListsBuf.put(SAMPLES_DISPLAY_LIST_POS,  
                               ...
                               topDisplayListsBuf.get(SAMPLES_DISPLAY_LIST_POS) : gl.glGenLists(1));
    ...
    public void recompileAxes(GLAutoDrawable drawable)
    {
        ...
        if (drawsAxes() &&
            ...
            double arrowSpread = 0.3d;
             drawArrow(gl, new double[] {getXMin(), 0.0d, 0.0d}, 
                       new double[] {getXMax(), 0.0d, 0.0d},
                       arrowLength, arrowSpread);
            drawBanner(gl, glut, 
                       new double[] {getXMax(), 0.0d, 0.0d},
                       getXParameter().getName());

             drawArrow(gl, new double[] {0.0d, getYMin(), 0.0d}, 
                       new double[] {0.0d, getYMax(), 0.0d},
                       arrowLength, arrowSpread);
            drawBanner(gl, glut, 
                       new double[] {0.0d, getYMax(), 0.0d},
                       getYParameter().getName());

             drawArrow(gl, new double[] {0.0d, 0.0d, getZMin()}, 
                       new double[] {0.0d, 0.0d, getZMax()},
                       arrowLength, arrowSpread);
            drawBanner(gl, glut, 
                       new double[] {0.0d, 0.0d, getZMax()},
    ...
    /**
     * Recompiles the list that draws the box at the minimum of the
     * plot.
     *
     * @param drawable the drawable on which to draw
     */
...
//                           vNorm);
            ...
            if (grid != null) {
                for (int i = 0; i < xTiles; i++) {
                    ...
                    for (int j = 0; j < yTiles + 1; j++) {
                        ...
                        gl.glVertex3d(grid[p1Index + 0], 
                                      grid[p1Index + 1], 
                                      grid[p1Index + 2]);
                        gl.glColor4dv(colorMapper.map(grid[p2Index + 3]), 0);
                        gl.glVertex3d(grid[p2Index + 0], 
                                      grid[p2Index + 1], 
                                      grid[p2Index + 2]);                        
                    }
                    gl.glEnd();
    ...
    /**
     * Called when the plot needs to be redrawn. Will redraw within
     * specified time.
     *
