...
/**
 * GradientSampler produces samples that lies in the direction of the gradient.
 * 
...
public class GradientSampler implements Sampler {
     ...
     * 
     * @param isMinimizing true if this sampler should seek to minimize the evaluations.
     */
    ...
    /**
     * Returns a new ParameterInstanceSet to sample. It is assumed that the provided evaluations contains at least one sample (in order for
     * the sampler to know what Parameters to create samples for) else it returns null.
     * 
     ...
     * 
     * @param evaluations previous samples, note that it is assumed that evaluations.size() >= 2.
     */
    ParameterInstanceSet createGradientNeighbour(Vector evaluations) {
        Hashtable<Parameter, Double> gradients = getBackwardDifference((Evaluation) evaluations
                                                                               .elementAt(evaluations
                                                                                       .size() - 2),
                                                                       (Evaluation) evaluations
                                                                               .elementAt(evaluations
                                                                                       .size() - 1));

        ParameterInstanceSet latest = ((Evaluation) evaluations
                .elementAt(evaluations.size() - 1)).getParameterInstanceSet();
...
//                 Debug.err.println("Bad gradient for " + parameter.getName() + ": " + gradient);
                ...
                result.add(parameter.getGradientNeighbour(latest
                        .getParameterInstance(parameter), gradient
                        .doubleValue()));
            }
        }
     ...
     */
    Hashtable<Parameter, Double> getBackwardDifference(Evaluation backward,
            Evaluation center) {
        Hashtable<Parameter, Double> res = new Hashtable<Parameter, Double>();

        // Calculate the difference of the evaluation.
        double eDiff = backward.getEvaluation().getParameter().getMetric()
                .d(backward.getEvaluation(), center.getEvaluation());
        eDiff *= ((double) backward.getEvaluation().getParameter()
                .getComparator()
                .compare(backward.getEvaluation(), center.getEvaluation()));

        for (Enumeration e = backward.getParameterInstanceSet()
                .getParameterInstances(); e.hasMoreElements();) {
            ParameterInstance instance = (ParameterInstance) e.nextElement();
            // Calculate the difference of each particular parameter.
            double pDiff = instance
                    .getParameter()
                    .getMetric()
                    .d(instance,
                       center.getParameterInstanceSet()
                               .getParameterInstance(instance.getParameter()));
            pDiff *= ((double) instance
                    .getParameter()
                    .getComparator()
                    .compare(instance,
                             center.getParameterInstanceSet()
...
//             Debug.err.println("eDiff = " + eDiff + " pDiff = " + pDiff + " gradient = " + gradient);
    ...
    ParameterInstanceSet createRandomNeighbour(Evaluation evaluation) {
        ...
        for (Enumeration e = result.getParameterInstances(); e
                .hasMoreElements();) {
            ParameterInstance parameterInstance = (ParameterInstance) e
                    .nextElement();
            double gradient = random.nextGaussian() - 0.5d;
            result.add(parameterInstance.getParameter()
                    .getGradientNeighbour(parameterInstance, gradient));
        }
        return result;
    }
}
