...
/**
 * Provides Evolver instances by collecting bits and pieces from Gui components.
 * 
...
public class GuiEvolverFactory extends JPanel implements EvolverFactory {
    ...
    public GuiEvolverFactory() {
        ...
        add(new JSplitPane(JSplitPane.VERTICAL_SPLIT,
                createTargetChoicePanel(), createParameterChoicePanel()));
    }
    ...
    public GuiEvolverFactory(StratmasObject root) {
        ...
        add(new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(
                    treeView), new JSplitPane(JSplitPane.VERTICAL_SPLIT,
                    createTargetChoicePanel(), createParameterChoicePanel())),
            BorderLayout.CENTER);
    }
    ...
    /**
     * Sets the Stopper factory used to create the stopper for evaluators.
     */
    ...
    /**
     * Returns the the Stopper factory used to create the stopper for evaluators.
     */
    ...
    public JPanel createParameterChoicePanel() {
        ...
        res.setDropTarget(new DropTarget(res, new DropSink() {
            ...
            public boolean hasParameter(Parameter parameter) {
                for (int i = 0; i < parameterTableModel.getRowCount(); i++) {
                    Object entry = parameterTableModel
                            .getValueAt(i, parameterTableModel
                                    .findColumn("Parameter"));
                    if (entry != null
        ...
        res.setDropTarget(new DropTarget(res, new DropSink() {
            ...
            public boolean checkDrop(final StratmasObject object) {
                ...
                if (object.getType().canSubstitute("Shape")
                    ...
                    ProcessVariableParameter pvParameter = new ProcessVariableParameter(
                            (Shape) object, null, null);
                    try {
                        ServerSession session = DefaultServerSession
                                .allocateSession();
                        if (session != null) {
                            session.open();
                ...
                if (parameter != null) {
                    if (!hasParameter()
                        ...
                        measureTableModel.setValueAt(getTargetParameter()
                                .toString(), 0, 0);
                        if (getEvaluatorStopperFactory() != null) {
                            res.remove((JComponent) getEvaluatorStopperFactory());
                        }

                        setEvaluatorStopperFactory(new GuiIterationsStopperFactory(
                                "Simulation") {
                            /**
									 * 
									 */
                            ...
                            public Stopper getStopper() {
                                return new IterationsStopper(getIterations()) {
                                     ...
                                     */
                                    public int getIterations(Object o) {
                                        return ((SimulationEvaluator) o)
                                                .getTarget().getUpdateCount();
                                    }
                                };
                            }
                        });
    ...
    public Evolver getEvolver() {
        ...
        if (evaluatorFactory != null && sampler != null && stopper != null) {
            return new Evolver(initialValues, targetParameter,
                    evaluatorFactory, sampler, stopper);
        } else {
            return null;
        }
    }
...
/**
 * class that provides a convinient minimal droptargetadapter to drop things in.
 */
        ...
        if (dtde.isDataFlavorSupported(StratmasObject.STRATMAS_OBJECT_FLAVOR)) {
            ...
            try {
                ...
                throw new AssertionError(
                        "Apparent inconsistency in DnD implementation");
            } catch (IOException e) {
