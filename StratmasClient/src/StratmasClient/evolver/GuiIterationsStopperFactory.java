...
/**
 * Provides Stopper instances by collecting bits and pieces from Gui components.
 * 
 * @version 1, $Date: 2005/11/01 16:50:47 $
 * @author Daniel Ahlin
 */
abstract public class GuiIterationsStopperFactory extends JPanel implements
        StopperFactory {
    /**
	 * 
	 */
    ...
    /**
     * Creates a new StopperFactory letting the user configure a IterationsStopper.
     * 
     * @param name the name of the component being stopped by this stoppers created by this class.
     */
    public GuiIterationsStopperFactory(String name) {
        setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
        SpinnerNumberModel spinnerModel = new SpinnerNumberModel(
                getIterations(), 0, Integer.MAX_VALUE, 1);
        JSpinner spinner = new JSpinner(spinnerModel);

        spinnerModel.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent event) {
                setIterations(((SpinnerNumberModel) event.getSource())
                        .getNumber().intValue());
            }
        });
        add(spinner);
        ...
        setBorder(BorderFactory.createCompoundBorder(BorderFactory
                .createTitledBorder(string + "Iterations"), BorderFactory
