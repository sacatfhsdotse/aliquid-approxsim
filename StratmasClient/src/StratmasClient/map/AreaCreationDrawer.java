 ...
 * circular. The existing area can be moved to another location. If the area is circular its radius can be changed. If it is polygonial the
 * vertices can be moved and the new ones can be inserted.
 * 
 * @version 1.0
...
public class AreaCreationDrawer extends BasicMapDrawer {
    ...
    public AreaCreationDrawer(BasicMap basicMap, Region region,
        ...
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .setSymbolOpacity(1.0);
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .setSymbolScale(1.0);
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .setLocationOpacity(1.0);
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .setDrawLocation(true);
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .setDrawLocationOutline(true);
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .setInvariantSymbolSize(true);
        ((MapElementAdapter) mapDrawableAdapters.get(sComp))
                .invalidateAllLists();
        // show gui
        createAndShowGUI(new String("Area definition for "
                + sComp.getType().getName() + " " + sComp.getIdentifier()));
    }
     ...
     * double. Part of MouseListener interface.
     * 
    ...
    /**
     * Used to move the points and shapes on the map and also to change the radius of the drawn circle while creating a new area. Part of
        ...
        if (panButton.isSelected()) {
            ...
            // represents one point which connects two lines.
                ...
                if (!mpAdapters.isEmpty()) {
                    ...
                    MapPointAdapter mpAdapter = (MapPointAdapter) mpAdapters
                            .firstElement();
    ...
    public void composeGUI() {
        ...
        circleButton = new JToggleButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "circle.png")));
        ...
        polygonButton = new JToggleButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "polygon.png")));
        ...
        newPointButton = new JToggleButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "newpoint.png")));
        ...
        newPointButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                if (self.getCreatorMode() != AreaCreationDrawer.INSERTPOINT) {
                    ...
                    Vector pointAdapters = self
                            .mapDrawableAdapters(MapPointAdapter.class);
        ...
        movePointButton = new JToggleButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "movepoint.png")));
        movePointButton
                .setToolTipText("Change the area by moving the desired point");
        ...
        moveShapeButton = new JToggleButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "hand.png")));
        ...
        deleteButton = new JButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "Delete16.gif")));
        ...
        cancelButton = new JButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "cancel.png")));
        ...
        acceptButton = new JButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "ok.png")));
        ...
        toolBar.setBorder(BorderFactory.createCompoundBorder(BorderFactory
                .createTitledBorder("Area Creation"), BorderFactory
                .createEmptyBorder(2, 2, 2, 2)));
        // panning button
        JToolBar panBar = new JToolBar();
        panButton = new JToggleButton(new ImageIcon(
                AreaCreationDrawer.class.getResource(path + "pan.png")));
        panBar.addSeparator();
        panBar.add(panButton);
        panBar.addSeparator();
        panBar.setBorder(BorderFactory.createCompoundBorder(BorderFactory
    ...
    protected void drawGraph(GLAutoDrawable gld) {
        ...
        if (getCircle() != null) {
            ...
            for (Enumeration e = pol.getCurves(); e.hasMoreElements();) {
                gl.glVertex2dv(proj.projToXY(((Line) e.nextElement())
                        .getStartPoint()), 0);
            }
            gl.glEnd();
    ...
    /**
     * Completes the polygonial by adding a line between the last and the first points in the list. If the polygonial cannot be created all
     * the points and lines are removed from the creator.
     */
    ...
    /**
     * Inserts new point into the polygonial. This method is used to insert a point into previously definied polygonial.
     * 
     * @param point the point to be inserted.
     * @param line the line which is divided by the inserted point into two new lines. This line is later replaced by the new lines.
     */
    public void insertPoint(Point point, Line line) {
        try {
            // replace the line
            MapDrawableAdapter lineAdapter = getMapDrawableAdapter(line
                    .getReference());
            if (lineAdapter != null) {
                // change identifiers for all the lines that come after the actual line
                int lineId = Integer.parseInt(lineAdapter.getObject()
                        .getIdentifier().toString());
                Vector lineAdapters = mapDrawableAdapters(MapLineAdapter.class);
                for (int i = 0; i < lineAdapters.size(); i++) {
                    MapLineAdapter ad = (MapLineAdapter) lineAdapters.get(i);
                    int id = Integer.parseInt(ad.getObject().getIdentifier()
                            .toString());
                    if (id > lineId) {
                        ((Line) ad.getObject()).setIdentifier(String
                                .valueOf(id + 1));
                    }
                }
    ...
    public void updateRadius(Point point) {
        ...
        Shape initShape = (sComp.getChild("location") != null) ? (Shape) sComp
                .getChild("location") : getTemporaryCircle();
    ...
    /**
     * Cretates a Circle with zero radius. This method is used to obtain the initial Circle for objects that don't have area.
     * 
    ...
    /**
     * Initializes the center of the area of the actual object. If the object is visible on the map then the center of it's BoundingBox is
     * the center of the area. If the object is Activity without location and it's owner is visible on the map then the center of the
     * owner's BoundingBox is the center of the area. In all other cases the center of the area is the center of the visible part of the
     * map.
     * 
    ...
    private Point getInitialCenter() {
        ...
        if (sComp.getChild("location") != null) {
            ...
            locBox = ((Shape) sComp.getParent().getParent()
        ...
        if (center != null && center.getLon() >= bbox.getWestLon()
                && center.getLon() <= bbox.getEastLon()
                && center.getLat() >= bbox.getSouthLat()
    ...
    public void moveShape(double lon, double lat) {
        ...
        if (box != null) {
            ...
            if (circle != null) {
                ...
                // polygonial area
                Vector lineAdapters = mapDrawableAdapters(MapLineAdapter.class);
                for (Enumeration e = lineAdapters.elements(); e
                        .hasMoreElements();) {
                    Line line = (Line) ((MapLineAdapter) e.nextElement())
                            .getObject();
    ...
    public BoundingBox getAreaBoundingBox() {
        ...
        if (!pointAdapters.isEmpty()) {
            Point p = (Point) ((MapPointAdapter) pointAdapters.firstElement())
                    .getObject();
            ...
            for (int i = 1; i < pointAdapters.size(); i++) {
                p = (Point) ((MapPointAdapter) pointAdapters.get(i))
                        .getObject();
    ...
    public Polygon createPolygonialShape() {
        ...
        if (lineAdapters.size() >= 3) {
            ...
            for (int i = 0; i < lineAdapters.size(); i++) {
                Line line = (Line) ((MapLineAdapter) lineAdapters.get(i))
                        .getObject();
    ...
    /**
     * Finds the "twin" Point of the given Point. In the polygonial shape the point where two lines meet represents of two Point objects.
     * This method finds the "other" Point object when one is given.
     * 
    ...
    public Point findMeetingPoint(Point p) {
        ...
        Vector lineAdapters = mapDrawableAdapters(MapLineAdapter.class);
        for (int i = 0; i < lineAdapters.size(); i++) {
            Line tmpLine = (Line) ((MapLineAdapter) lineAdapters.get(i))
                    .getObject();
            Point tmpPoint = (startPoint) ? tmpLine.getEndPoint() : tmpLine
                    .getStartPoint();
            if (tmpPoint.getLat() == p.getLat()
                    && tmpPoint.getLon() == p.getLon()) {
