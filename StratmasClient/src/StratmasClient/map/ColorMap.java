 ...
 * equivalent values are displayed. These values depend on how the scale is interpreted ie., linearly or logarithmicaly. Further on, the
 * numerical values depends on the upper and lower bounds for the scale.
 * <p>
 * The scale (linear or logarithmic), the upper and lower bounds and the color map can be chosen from the dialog implemented in
 * <code>ColorMapDialog</code> class.
 * 
 * @see ColorMapDialog
 * @version 1.0
...
public class ColorMap implements GLEventListener, ActionListener {
    ...
    private void createColorMapPanel() {
        ...
        properties_button.setFont(properties_button.getFont()
        ...
        colorMapPanel.setBorder(BorderFactory
                .createCompoundBorder(BorderFactory
                        .createTitledBorder("Color Map"), BorderFactory
    ...
    /**
     * Update color map table with user defined color map table. Number of shades is currently 256 where each shade consists of it's red,
     * green and blue components.
     * 
     * @param ctable array of 256 shades where each shade consists of its red, green and blue components. The shades are scaled such that
     *            each component has to lie between 0.0 and 1.0 values (inclusive these two).
     * @throws RuntimeException 1.if the dimension of <code>ctable</code> is unvalid ie. it is not [256][3], 2.if elements of
     *             <code>ctable</code> are not in the range 0.0 - 1.0.
     */
    ...
    /**
     * Update color map table with a given combination of colors. Minimum number of colors is two. Transition between colors is computed by
     * using Hermite interpolation with slope zero.
     * 
     * @param cmap array of colors (between 2 and 256) where each color consists of its red, green and blue components. The shades are
     *            scaled such that each component has to lie between 0.0 and 1.0 values (inclusive these two).
     * @throws RuntimeException 1.if the dimension of <code>ctable</code> is unvalid ie. it is not [X][3] with 1<X<257, 2.if elements of
     *             <code>ctable</code> are not in the range 0.0 - 1.0.
     */
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        try {
            ...
            if (ae.getSource().equals(properties_button)) {
                Vector v = ColorMapDialog.showDialog(frame, properties_button,
                                                     actual_scale, min_value,
                                                     max_value, color_map_name);
                if (v != null && !v.isEmpty()) {
                    // update scale
                    ...
                    if (actual_scale.equals("Linear Scale")) {
                        displayed_values = selectLinearScaleTable(min_value,
                                                                  max_value);
                        // update the current process variable
                        if (actual_pv != null) {
                        ...
                        displayed_values = selectLogarithmicScaleTable(min_value,
                                                                       max_value);
                        // update the current process variable
                        if (actual_pv != null) {
    ...
    /**
     * Compute logarithmic scale table. It is assumed that minimum value is nonnegative and maximum value is larger then minimum value.
     * 
    ...
    private float[] selectLogarithmicScaleTable(float min, float max) {
        ...
        if (nr_of_selected_values > 2
                && Math.abs(Math.log(max) / Math.log(10)
                        - Math.log(tmp_table[nr_of_values - 2]) / Math.log(10)) < 0.2) {
        ...
        if (nr_of_selected_values > 2
                && Math.abs(Math.log(tmp_min) / Math.log(10)
                        - Math.log(tmp_table[1]) / Math.log(10)) < 0.2) {
            tmp_table[1] = -1.0f;
            nr_of_selected_values--;
        }
        int tmp = 2;
    ...
    private void drawString(GL gl2, int x, int y, float f) {
        ...
        int lng = (int) (xscale * glut
                .glutBitmapLength(GLUT.BITMAP_HELVETICA_10, s));
    ...
    /**
     * Scale given value such that it maps to color scale. It is assumed that minimum value is nonnegative if logarithmic scale is used.
     * Zero value is scaled to 0.1 in logarithmic scale. Further, it is assumed that maximum value is larger then minimum value for both
     * scales.
     * 
    ...
    public int scaleValue(float value) {
        ...
        // logarihmic scale
            ...
            max_value = (max_value > 100000000000.0f) ? 100000000000.0f
                    : max_value;
            //
    ...
    /**
     * Returns the color for the given value. The color is computed according to the actual scale, bounds and color map.
     * 
    ...
    public Color getMappingColor(double value) {
        ...
        return new Color(color_table[scaledValue][0],
                color_table[scaledValue][1], color_table[scaledValue][2]);
    }

    /**
     * Returns the actual scaled pv layer. OBS! It has meaning only if used together with the current color table.
     * 
    ...
    private void drawGraph(GL gl2) {
        ...
        // draw color map
        int ii = 0;
        for (int x = xlabel_min; x < xlabel_max; x = x + xlabel_max
                / color_shades) {
        ...
        gl.glVertex2d(xlabel_max, ymax / 2);
        gl.glEnd();
        drawString(gl, xlabel_max, ymin,
                   displayed_values[displayed_values.length - 1]);
    }
