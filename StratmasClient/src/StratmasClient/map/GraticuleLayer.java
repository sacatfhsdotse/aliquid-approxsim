...
/**
 * The graticules which cover a region inside a bounding box are created. Density of graticules can be set to 10, 5 and 1 degree(s).
 * 
 * @version 1.0
...
public class GraticuleLayer {
     ...
     */
    public static void drawGraticules(GL gl2, BoundingBox box, Projection proj,
            double spacing) {
        GL2 gl = (GL2) gl2;
        // increase bounding box for the graticules
        double westLon = box.getWestLon()
                - (box.getEastLon() - box.getWestLon());
        double eastLon = box.getEastLon()
                + (box.getEastLon() - box.getWestLon());
        double southLat = box.getSouthLat()
                - (box.getNorthLat() - box.getSouthLat());
        double northLat = box.getNorthLat()
                + (box.getNorthLat() - box.getSouthLat());
        double step = 0.1d;
