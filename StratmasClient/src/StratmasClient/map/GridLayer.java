...
public class GridLayer implements StratmasEventListener {
    ...
    /**
     * Indicator for the active cells in the grid. If a cell covers the actual region it's value is true otherwise it's value is false.
     */
    ...
    /**
     * Returns true if the given name is the render selection name of any displayed cell in the grid.
     */
    ...
    public void updateActiveCells() {
        ...
        for (int i = 0; i < gridData.getActiveCells().length; i++) {
            ...
            if (displayedCellCounter < nrOfActiveCells) {
                ...
                // update the counter
                activeCellCounter = (activeInGrid) ? activeCellCounter + 1
                        : activeCellCounter;
                // active cells only
                if (activeInGrid && activeInRegion) {
                    ...
                    cell.renderSelectionName = getRenderSelectionName() + 1
    ...
    public Shape getCellShape(Cell cell) {
        ...
        if (Configuration.getCoordinateSystem() == Configuration.MGRS) {
            ...
            id = new String("cell at lat = " + lats
                    + MapConstants.DEGREE_SYMBOL + ", lon = " + lons
    ...
    /**
     * Returns the circle located att the center of the cell with small radius. This method is used for communication with the server.
     * 
    ...
    protected void updateDisplayList(Projection proj, GLAutoDrawable gld) {
        ...
        if (grid_values != null) {
            ...
            for (int ii = 0; ii < cellInfo.length; ii++) {
                ...
                // get the points
                double[] upper_left = proj
                        .projToXY(pos[2 * (row * (col_nr + 1) + col) + 1],
                                  pos[2 * (row * (col_nr + 1) + col)]);
                double[] lower_left = proj.projToXY(pos[2 * ((row + 1)
                        * (col_nr + 1) + col) + 1], pos[2 * ((row + 1)
                        * (col_nr + 1) + col)]);
                double[] upper_right = proj.projToXY(pos[2 * (row
                        * (col_nr + 1) + col + 1) + 1], pos[2 * (row
                        * (col_nr + 1) + col + 1)]);
                double[] lower_right = proj.projToXY(pos[2 * ((row + 1)
                        * (col_nr + 1) + col + 1) + 1], pos[2 * ((row + 1)
                        * (col_nr + 1) + col + 1)]);
                // draw the cell
                gl.glMatrixMode(GL2.GL_MODELVIEW);
                gl.glPushMatrix();
                ...
                gl.glColor3f(color_table[col_ind][0], color_table[col_ind][1],
                             color_table[col_ind][2]);
