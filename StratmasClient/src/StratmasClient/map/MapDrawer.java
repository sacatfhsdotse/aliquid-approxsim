 ...
 * cells, elements, graticules etc. currently displayed are handled here. <br>
 * The map is rendered by using OpenGL binding for Java (JOGL). <code> GLCanvas </code> is 
 * used as a drawing area. <p>
 * Drag'n'drop is supported in this class. All <code>StratmasObject</code> elements with location
 * can be dropped on the map. Further on, elements of type "MilitaryUnits" and "AgencyTeams" can 
 * be both dragged and dropped on the map.
 *
 * @version 1.0
...
public class MapDrawer extends BasicMapDrawer implements DragGestureListener, StratmasEventListener
{
    ...
    /**
     * Whether to render a magnification of the symbols currently
     * rendered under the mouse cursor.
     */
     ...
     * button (left or right), if the click is simple or double and the current mode of the map.
    ...
    /**
     * Used to move points and shapes on the map and also to change the radius of the drawn
    ...
    /**
     * Upadates the curent position on the map in (lat,lon) coordinates.
     ...
     *
     * @return true if the element is registred on the map as MapDrawableAdapter,
     *         false otherwise.
     */
    ...
    /**
     * Returns the MapElementAdapter for the given StratmasObject. Null is returned if the 
     ...
     *
     * @param ok if true the process variables are displayed as a grid of cells,
    ...
    /**
     * Returns true if the representation of the displayed process variable is grid based, 
     * false otherwise.
     */
    ...
    /**
     * Returns a vector with StratmasObject inherited elements presently drawn under the cursor. 
    ...
    /**
     * Sets flag indicating whether a panel showing a magnification of
     * the symbols currently rendered under the cursor should be
     * drawn.
     *
     * @param flag
     */
    ...
    /**
     * Returns true if a panel showing a magnification of the symbols
