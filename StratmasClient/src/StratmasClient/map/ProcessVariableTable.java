...
public class ProcessVariableTable extends JTable implements MouseListener,
     ...
     */
    public static final Color[] colors = { Color.BLUE, Color.RED, Color.GRAY,
            Color.MAGENTA, Color.ORANGE, Color.CYAN, Color.PINK, Color.YELLOW,
            Color.GREEN };
    /**
     * The used colors.
     */
    ...
    private void initColumns() {
        ...
        for (int i = 2; i < getModel().getColumnCount(); i++) {
            ...
            // get new color
            Color color = (j < colors.length) ? colors[j] : new Color(
    ...
    /**
     * Converts number to string using different decimal formats depending on the type of the number (Double or Integer) and it's size.
     * 
     ...
     */
    public ProcessVariableXYGraph createGraph(ProcessVariableDescription pv,
            String[] fac, Color[] colors) {
        // create a graph
        ProcessVariableXYGraph graph = new ProcessVariableXYGraph(
                client.getTimeline(), actualValues, pv, fac, colors, regionId);
     ...
     */
    public static void layoutGraph(ProcessVariableXYGraph graph,
            ProcessVariableTable pvTable) {
        boolean done = false;
        ...
        for (int i = 0; i < Visualizer.locationGrid.length; i++) {
            for (int j = 0; j < Visualizer.locationGrid[0].length; j++) {
                if (!done && Visualizer.locationGrid[i][j] == null) {
                    ...
                    Rectangle graphRectangle = new Rectangle(x, y,
                            ProcessVariableXYGraph.DEFAULT_WIDTH,
                            ProcessVariableXYGraph.DEFAULT_HEIGHT);
                    Rectangle thisRectangle = new Rectangle(
                            (int) pvTable.getTopLevelAncestor()
                                    .getLocationOnScreen().getX(),
                            (int) pvTable.getTopLevelAncestor()
                                    .getLocationOnScreen().getY(), pvTable
                                    .getTopLevelAncestor().getWidth(), pvTable
                                    .getTopLevelAncestor().getHeight());
    ...
    public static void layoutAllGraphs(ProcessVariableTable pvTable) {
        ...
        for (int i = 0; i < Visualizer.getGraphs().size(); i++) {
            layoutGraph((ProcessVariableXYGraph) Visualizer.getGraphs().get(i),
                        pvTable);
        }
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getClickCount() == 2) {
            ...
            if (col != getColumnModel().getColumnIndex("Category")) {
                ...
                if (col == getColumnModel().getColumnIndex("PV")
                        && pv.hasFactions()) {
            ...
            // create graphs for all process variables within the category with all factions
                ...
                Vector pvs = model.getProcessVariables((String) model
                        .getValueAt(row, col));
                for (int i = 0; i < pvs.size(); i++) {
                    ProcessVariableDescription pv = (ProcessVariableDescription) pvs
                            .get(i);
                    if (pv.hasFactions()) {
                        layoutAllFactionsGraph(pv);
                    } else {
     ...
     */
    private void layoutOneFactionGraph(ProcessVariableDescription pv,
            String faction, Color color) {
        String[] fac = { faction };
        Color[] colors = { color };
