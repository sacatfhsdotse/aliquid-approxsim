...
public class Visualizer {
    ...
    /**
     * Saves prameters for all opened graphs. These parameters are used to re-open the graphs later.
     */
    ...
    public static void reOpenGraphs() {
        while (!savedGraphParameters.isEmpty()) {
            ...
            ProcessVariableDescription pv = client
                    .getProcessVariable((String) param.get("processVariable"));
            if (pv != null) {
                ...
                ProcessVariableXYGraph graph = new ProcessVariableXYGraph(
                        (Timeline) param.get("timeline"), regionData, pv,
    ...
    /**
     * Saves prameters for all opened tables. These parameters are used to re-open the tables later.
     */
    ...
    public void reset() {
        ...
        // reset the tables
        for (Enumeration<ProcessVariableTablePanel> e = pvTables.elements(); e
                .hasMoreElements();) {
            e.nextElement().reset();
        }
    }
    ...
    /**
     * Adds a listener to a RegionData object. If the object doesn't exists, a new one is created and subscribed to the server.
     * 
    ...
    /**
     * Removes a listener from a Region Data object. If the object has no more listeners it is removed from the list of RegionData objects.
     * 
    ...
    public static void remove() {
        // remove the maps
        for (Enumeration<StratMap> e = stratmaps.elements(); e
                .hasMoreElements();) {
