 ...
 * 1. Zooming feature - the region can be zoomed from the largest scale, which is
 *    the longitude and latitude of its bounds obtained direct from the ESRI file, to the
 *    smallest scale, which is the size of one cell. <br>
...
public class ZoomAndScale implements ActionListener, ChangeListener {
    ...
    private void createZoomAndScalePanel(int orientation) {
        ...
        zpanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Zooming"),
        ...
        spanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Map Scale"),
                                                            BorderFactory.createEmptyBorder(2, 2, 2, 2)));
    ...
    /**
     * Computes new orthographic scaled bounds for the main map and updates the map scale.
     * Used when zooming slider changes.
     *
    ...
    private void setOrthoBounds(double i) {
        ...
        // update the main map
        drawer.setScaledBoundingBox(new BoundingBox(ort_xmins, ort_ymins, ort_xmaxs, ort_ymaxs, drawer.getProjection()));
        // compute constants for scaling function
        a = drawer.getMinRange() / dx1 - 1.0;
        // update screen distance
        dist_screen = drawer.getViewWidth()*MapConstants.ONE_INCH / Toolkit.getDefaultToolkit().getScreenResolution();
    ...
    private int compScale(double xmin, double ymin, double xmax, double ymax) {
        // distance on the surface of the earth (km)
         double dist_earth = drawer.getProjection().getDistanceGC(xmin, ymin, xmax, ymax);
    ...
    public void actionPerformed (ActionEvent ae) {
        ...
        try {
            ...
            // parse the scale number
                ...
                double i = Math.pow((act_range - max_range) / (min_range - max_range), 4);
    ...
    /**
     * Action is fired when the slider status is changed. The main map is 
     * updated accordingly.
     *
