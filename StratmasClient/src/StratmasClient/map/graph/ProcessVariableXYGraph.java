 ...
 * The used scale can be linear or logarithmic.
 * 
 * @version 1.0
...
public class ProcessVariableXYGraph extends JPanel implements
    ...
    /**
     * The middle y-values used when the scale is logarithmic and the diiference between the bounds is one power of ten.
     */
     ...
     */
    public ProcessVariableXYGraph(Timeline timeline, RegionData regionData,
            ProcessVariableDescription processVariable, String[] factions,
        ...
        startTime = (regionData.getTimestamp() == null) ? 0
                : getRelativeGraphTime(regionData.getTimestamp().getMilliSecs());
        ...
        // set the title
        JPanel titlePanel = DisplayControl.getTitlePanel(processVariable
                .getName(), factions, factionColors, regionId);
    ...
    private JPanel createGraphPanel() {
        ...
        String yAxisName = (isLinearScale()) ? new String(
                processVariable.getName()) : new String("Log("
        ...
        yScalePanel.addComponentListener(new ComponentAdapter() {
            public void componentResized(ComponentEvent e) {
                tmpLabel.setPreferredSize(new Dimension(
                        ((JPanel) e.getSource()).getWidth(), 1));
                tmpLabel.invalidate();
                xPanel.validate();
            }
        });
    ...
    public void reset() {
        ...
        startTime = getRelativeGraphTime(regionData.getTimestamp()
                .getMilliSecs());
        endTime = startTime + 20;
        // reset the values
        // previousValues.remove();
    ...
    public void eventOccured(ApproxsimEvent e) {
        if (e.getSource() instanceof RegionData) {
            ...
            long actTime = ((RegionData) e.getSource()).getTimestamp()
                    .getMilliSecs();
            // get actual values
            Hashtable val = ((RegionData) e.getSource()).getPV(processVariable
                    .getName());
    ...
    private void setInitialYBounds() {
        ...
        if (isLogarithmicScale()) {
            ...
            lowerYBound = (lowerYBound <= 0) ? 0
                    : ((lowerYBound < MINIMUM_LOGARITHMIC_Y_VALUE) ? MINIMUM_LOGARITHMIC_Y_VALUE
                            : lowerYBound);
            upperYBound = (upperYBound > MAXIMUM_LOGARITHMIC_Y_VALUE) ? MAXIMUM_LOGARITHMIC_Y_VALUE
                    : upperYBound;
            ...
            if (lowerYBound == 0) {
                ...
                if (upperYBound >= 10) {
                    ...
                    nrOfDispVal = (int) Math.round(log10(upperYBound)
                            - log10(MINIMUM_LOGARITHMIC_Y_VALUE)) + 2;
                }
                nrOfDispVal = (nrOfDispVal > NR_OF_DISPLAYED_Y_VALUES) ? NR_OF_DISPLAYED_Y_VALUES
                        : nrOfDispVal;
            ...
            // the lower bound is positive
                ...
                int nrOfDispVal = (int) Math.round(log10(upperYBound)
                        - log10(lowerYBound)) + 1;
                nrOfDispVal = (nrOfDispVal > NR_OF_DISPLAYED_Y_VALUES) ? NR_OF_DISPLAYED_Y_VALUES
                        : nrOfDispVal;
                secondLowerYBound = (nrOfDispVal == 2) ? lowerYBound * 5
                        : lowerYBound * 10;
                setNrOfDisplayedYValues((nrOfDispVal == 2) ? MIDDLE_LOG_VALUES.length + 2
                        : nrOfDispVal);
            }
        }
        // the linear scale
     ...
     * 
     * @param visible if true the legend becomes visible, if false the legend becomes hidden.
     */
    ...
    public void update(Hashtable values, long actTime) {
        ...
        double minVal = (isLinearScale()) ? currentValues.getMinValue()
                : currentValues.getMinNonNegativeValue();
        double maxVal = currentValues.getMaxValue();

        if (minVal <= maxVal) {
            // check if the values are inside the allowed bounds
            minVal = (minVal <= processVariable.getMin()) ? processVariable
                    .getMin()
                    : (minVal >= processVariable.getMax()) ? processVariable
                            .getMax() : minVal;
            maxVal = (maxVal >= processVariable.getMax()) ? processVariable
                    .getMax()
                    : (maxVal <= processVariable.getMin()) ? processVariable
                            .getMin() : maxVal;

            // update the bounds on the y-axis
            if (minVal < lowerYBound || maxVal > upperYBound
                    || !initializedYBounds) {
                // linear scale
                if (isLinearScale()) {
    ...
    public void updateVerticalAxisLinearBounds(double minVal, double maxVal) {
        ...
        if (minVal == maxVal) {
            double diff = Math.pow(10,
                                   Math.ceil(log10(processVariable.getMax()
            ...
            if (lowerYBound < processVariable.getMin()) {
                lowerYBound = processVariable.getMin();
                upperYBound = (upperYBound < lowerYBound + yMarg) ? lowerYBound
                        + yMarg : upperYBound;
            } else if (upperYBound > processVariable.getMax()) {
                upperYBound = processVariable.getMax();
                lowerYBound = (lowerYBound > upperYBound - yMarg) ? upperYBound
                        - yMarg : lowerYBound;
            }
        }
        // the maximum and the minimum of the displayed values are not equal
            ...
            if (yMarg >= processVariable.getMax() - processVariable.getMin()) {
                ...
                if (lowerYBound + yMarg < maxVal) {
                    yMarg = (Math.pow(10, Math.ceil(log10(yMarg))) == yMarg) ? yMarg * 5
                            : yMarg * 2;
                }
                // set the upper bound
    ...
    public void updateVerticalAxisLinearBounds() {
        ...
        double maxVal = currentValues.getMaxValue();
        if (getShowPrevious()) {
            minVal = (previousValues.getMinValue() < minVal) ? previousValues
                    .getMinValue() : minVal;
            maxVal = (previousValues.getMaxValue() > maxVal) ? previousValues
                    .getMaxValue() : maxVal;
        }
    ...
    public void updateVerticalAxisLogarithmicBounds(double minVal, double maxVal) {
        ...
        if (minVal < lowerYBound || !initializedYBounds) {
            lowerYBound = (minVal == 0) ? 0 : Math.pow(10, Math
                    .floor(log10(minVal)));
        }
        ...
        if (lowerYBound == upperYBound) {
            if (lowerYBound > 0) {
                ...
                upperYBound = (processVariable.getMax() >= 100) ? 100
                        : processVariable.getMax();
                int nrOfDispVal = (int) Math.round(log10(upperYBound)
                        - log10(MINIMUM_LOGARITHMIC_Y_VALUE)) + 2;
                setNrOfDisplayedYValues((nrOfDispVal > NR_OF_DISPLAYED_Y_VALUES) ? NR_OF_DISPLAYED_Y_VALUES
                        : nrOfDispVal);
                secondLowerYBound = upperYBound
                        / Math.pow(10, NR_OF_DISPLAYED_Y_VALUES - 2);
            }
        }
        // the maximum bound is different from the minimum bound
        else {
            int nrOfDispVal = (lowerYBound == 0) ? (int) Math
                    .round(log10(upperYBound)
                            - log10(MINIMUM_LOGARITHMIC_Y_VALUE)) + 2
                    : (int) Math.round(log10(upperYBound) - log10(lowerYBound)) + 1;
            nrOfDispVal = (nrOfDispVal > MAX_NR_OF_DISPLAYED_Y_VALUES) ? MAX_NR_OF_DISPLAYED_Y_VALUES
                    : nrOfDispVal;
    ...
    public void updateVerticalAxisLogarithmicBounds() {
        ...
        double maxVal = currentValues.getMaxValue();
        if (getShowPrevious()) {
            minVal = (previousValues.getMinNonNegativeValue() < minVal) ? previousValues
                    .getMinNonNegativeValue() : minVal;
            maxVal = (previousValues.getMaxValue() > maxVal) ? previousValues
                    .getMaxValue() : maxVal;
        }
        // set the number of displayed values on the y-axis
    ...
    /**
     * Writes the series as a semicolon separated list to the provided stream.
     * 
        ...
        if (!showPrevious) {
            ...
            if (previousValues != null) {
                for (int j = 0; j < previousValues.times.size(); j++) {
                    long time = ((Long) previousValues.times.get(j))
                            .longValue();
                    String line = "Previous;"
                            + format.format(new java.util.Date(time));
                    for (int i = 0; i < factions.length; i++) {
                        // get value
                        Hashtable htable = (Hashtable) previousValues.values
                                .get(j);
                        Number val = (Number) htable.get(factions[i]);
                        if (val != null) {
                            line += ";" + val.toString();
    ...
    void exportSeries() {
        try {
            String filename = Client
                    .getFileNameFromDialog(".csv", JFileChooser.SAVE_DIALOG,
                                           frame);
            if (filename != null) {
    ...
    void saveImage() {
        ...
        if (retVal == JFileChooser.APPROVE_OPTION) {
            ...
            String extension = (new String("."))
                    .concat(((FileExtensionFilter) chooser.getFileFilter())
                            .getExtension());
            ...
            Rectangle rectangle = new Rectangle((int) loc.getX(),
                    (int) loc.getY(), this.getWidth(), this.getHeight());
...
class SimulationPVValues {
    ...
    /**
     * Adds new values and the time when these values are obtained to the lists.
     * 
    ...
    public void add(Hashtable value, long time) {
        ...
        minNonNegativeValue = (minVal >= 0 && minVal < minNonNegativeValue) ? minVal
                : minNonNegativeValue;
        for (int i = 1; i < factions.length; i++) {
            ...
            minNonNegativeValue = (tmpValue >= 0 && tmpValue < minNonNegativeValue) ? tmpValue
                    : minNonNegativeValue;
        }
        if (initialized) {
