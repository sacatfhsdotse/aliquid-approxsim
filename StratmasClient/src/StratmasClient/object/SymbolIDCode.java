...
public class SymbolIDCode extends ApproxsimSimple {
     ...
     */
    protected SymbolIDCode(String identifier, Type type, String value)
     ...
     */
    protected SymbolIDCode(Declaration declaration, String value)
            throws ParseException {
        super(declaration);
        setValue(value, this);
    }
    ...
    private void setValue(String newValue, Object initiator)
        ...
        if (trimmedValue.length() != 15) {
            throw new ParseException("'" + trimmedValue
                    + "' is an illegal Symbol ID code", 15);
        ...
        try {
            return new SymbolIDCode(Identifier.getIdentifier(n),
                    TypeFactory.getType(n), XMLHelper.getString(XMLHelper
                            .getFirstChildByTag(n, "value"), "value"));
        } catch (ParseException e) {
            throw new AssertionError(
                    "Error creating SymbolIDCode from DOM element: "
                            + e.getMessage());
        }
    }
    ...
    public void update(Element n, Timestamp t) {
        try {
            valueFromString(XMLHelper.getString(XMLHelper
                    .getFirstChildByTag(n, "value"), "value"), n);
        } catch (ParseException e) {
...
/**
 * SymbolIDCodeGUIConstructor creates GUIs for creating SymbolIDCode objects.
 * 
 * @version 1, $Date: 2006/05/16 09:10:48 $
 * @author Daniel Ahlin
 */
