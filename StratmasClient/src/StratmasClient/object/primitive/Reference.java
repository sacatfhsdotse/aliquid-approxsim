...
/**
 * A reference is used to reference an identifiable object (the resolve mechanism is scope dependent).
 * 
...
public class Reference {
    ...
    public Reference(Reference scope, String id) {
        if (scope == null) {
            ...
            identifiers[0] = new String(id);
            System.arraycopy(scope.identifiers, 0, identifiers, 1,
                             scope.identifiers.length);
        }
    }
    ...
    /**
     * Returns object referenced by this reference. Tries to resolve the reference in every scope up to the root.
     * 
        ...
        for (StratmasObject currentScope = scope; currentScope != null; currentScope = currentScope
            ...
            if (candidate == null
                    && currentScope.getIdentifier()
                            .equals(identifiers[identifiers.length - 1])) {
            ...
            if (candidate != null) {
                ...
                for (int i = identifiers.length - 2; i >= 0
                        && candidate != null; i--) {
    ...
    /**
     * Returns object referenced by this reference. Tries to resolv the reference in every scope up to the root.
     * 
    ...
    public StringBuffer bodyXML(StringBuffer b) {
        ...
        if (identifiers.length == 0) {
            ...
            b.append("<name>")
                    .append(XMLHelper.encodeSpecialCharacters(identifiers[0]
                                    .toString())).append("</name>");
            for (int i = 1; i < identifiers.length; i++) {
                b.append("<scope>")
                        .append("<name>")
                        .append(XMLHelper
                                        .encodeSpecialCharacters(identifiers[i]
                                                .toString())).append("</name>");
            }
            for (int i = 1; i < identifiers.length; i++) {
                b.append("</scope>");
    ...
    /**
     * Checks for equality. Notice that it is guaranteed that the hashcode for two References A and B is equal when A.equals(B) returns
     * true.
     * <p>
     * author Per Alexius
     * 
    ...
    /**
     * Tries to create a new reference from the provided string. Returns null iff the supplied string is empty or if any component (i. e.
     * scope) is defined to be empty (e. g. a::b or :). FIXME: should throw suitable excpetion instead of returning null on error.
     * 
...
//                                str.substring(currentStart, currentEnd) + "\"");
    ...
    /**
     * Returns the length of the reference (i. e. number of components)
     */
