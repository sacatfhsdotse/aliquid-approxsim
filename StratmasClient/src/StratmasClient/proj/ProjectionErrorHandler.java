...
public class ProjectionErrorHandler {
    ...
    public static void handleError(long error) {
        if (error == LAT_ERROR) {
            ...
            System.err
                    .println("ERROR : Origin latitude outside of valid range ");
        } else if (error == CENT_MER_ERROR) {
            System.err
                    .println("ERROR : Central meridian outside of valid range ");
        } else if (error == SCALE_FACTOR_ERROR) {
            System.err.println("ERROR : Scale factor outside of valid range ");
        } else if (error == A_ERROR) {
            System.err
                    .println("ERROR : Semi-major axis less than or equal to zero ");
        } else if (error == INV_F_ERROR) {
            System.err
                    .println("ERROR : Inverse flattening outside of valid range ");
        } else if (error == UTM_ZONE_ERROR) {
            System.err
                    .println("ERROR :  Zone outside of valid range (1 to 60) ");
        } else if (error == HEMISPHERE_ERROR) {
            System.err.println("ERROR :  Invalid hemisphere ");
        } else if (error == UTM_ZONE_OVERRIDE_ERROR) {
            System.err
                    .println("ERROR :  Zone outside of valid range (1 to 60) and within 1 of 'natural' zone ");
        } else if (error == MGRS_STR_ERROR) {
            System.err
                    .println("ERROR : MGRS string is too long, too short, or badly formed ");
        } else if (error == MGRS_PRECISION_ERROR) {
            System.err
                    .println("ERROR : MGRS precision must be between 0 and 5 inclusive ");
