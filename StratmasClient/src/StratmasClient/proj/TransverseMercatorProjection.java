...
/**
 * This is implementation of the Transverse Mercator projection where (longitude/latitude) values are converted into (easting/northing)
...
class TransverseMercatorProjection {
    ...
    /*
     * This method receives the Tranverse Mercator projection parameters as inputs, and sets the corresponding state variables.
     ...
     */
    public static boolean setTransverseMercatorParameters(
        ...
        // Origin latitude out of range
            ...
            ProjectionErrorHandler
                    .handleError(ProjectionErrorHandler.ORIGIN_LAT_ERROR);
            return false;
        }
        // Origin longitude out of range
        else if (central_meridian < -Math.PI
                || central_meridian > (2 * Math.PI)) {
            ProjectionErrorHandler
                    .handleError(ProjectionErrorHandler.CENT_MER_ERROR);
            return false;
        }
        // Invalid scale factor
            ...
            ProjectionErrorHandler
                    .handleError(ProjectionErrorHandler.SCALE_FACTOR_ERROR);
            return false;
        }

        // Set the maximum variance for easting and northing values
        double[] east_north = convertGeodeticToTransverseMercator(MAX_DELTA_LON,
                                                                  MAX_LAT);
        if (east_north == null) {
            return false;
        }
    ...
    private static double SPHTMD(double latitude) {
        return (double) (ProjectionConstants.ap * latitude
                - ProjectionConstants.bp * Math.sin(2.e0 * latitude)
                + ProjectionConstants.cp * Math.sin(4.e0 * latitude)
                - ProjectionConstants.dp * Math.sin(6.e0 * latitude) + ProjectionConstants.ep
                * Math.sin(8.e0 * latitude));
    }
    ...
    private static double SPHSR(double latitude) {
        return (double) (ProjectionConstants.semiMajorAxis
                * (1.e0 - Math.pow(ProjectionConstants.eccentricity, 2)) / Math
    ...
    private static double DENOM(double latitude) {
        return (double) (Math.sqrt(1.e0
                - Math.pow(ProjectionConstants.eccentricity, 2)
                * Math.pow(Math.sin(latitude), 2)));
    }

    /*
     * This method converts geodetic (latitude and longitude) coordinates to Transverse Mercator projection (easting and northing)
     ...
     */
    public static double[] convertGeodeticToTransverseMercator(
        ...
        if (Math.abs(dlam) > (9.0 * Math.PI / 180)) {
            ProjectionErrorHandler
                    .handleWarning(ProjectionErrorHandler.LON_WARNING);
        }
        ...
        double tan5 = tan4 * t;
        double tan6 = tan5 * t;
        double eta = ((1 / (1 - Math.pow(ProjectionConstants.eccentricity, 2))) - 1)
                * c2;
        double eta2 = eta * eta;
        ...
        double t3 = sn * s * c3 * tranMerc_scale_factor
        ...
        double t4 = sn
                ...
                * (61.e0 - 58.e0 * tan2 + tan4 + 270.e0 * eta - 330.e0 * tan2
                        * eta + 445.e0 * eta2 + 324.e0 * eta3 - 680.e0 * tan2
                        * eta2 + 88.e0 * eta4 - 600.e0 * tan2 * eta3 - 192.e0
                        * tan2 * eta4) / 720.e0;

        double t5 = sn * s * c7 * tranMerc_scale_factor
                * (1385.e0 - 3111.e0 * tan2 + 543.e0 * tan4 - tan6) / 40320.e0;

        double northing = tranMerc_false_northing + t1 + Math.pow(dlam, 2.e0)
                * t2 + Math.pow(dlam, 4.e0) * t3 + Math.pow(dlam, 6.e0) * t4
                + Math.pow(dlam, 8.e0) * t5;

        // easting
        double t6 = sn * c * tranMerc_scale_factor;
        double t7 = sn * c3 * tranMerc_scale_factor * (1.e0 - tan2 + eta)
                / 6.e0;
        double t8 = sn
                * c5
                * tranMerc_scale_factor
                * (5.e0 - 18.e0 * tan2 + tan4 + 14.e0 * eta - 58.e0 * tan2
                        * eta + 13.e0 * eta2 + 4.e0 * eta3 - 64.e0 * tan2
                        * eta2 - 24.e0 * tan2 * eta3) / 120.e0;
        double t9 = sn * c7 * tranMerc_scale_factor
                * (61.e0 - 479.e0 * tan2 + 179.e0 * tan4 - tan6) / 5040.e0;

        double easting = tranMerc_false_easting + dlam * t6
                + Math.pow(dlam, 3.e0) * t7 + Math.pow(dlam, 5.e0) * t8
                + Math.pow(dlam, 7.e0) * t9;
    ...
    /*
     * This method converts Transverse Mercator projection (easting and northing) coordinates to geodetic (latitude and longitude)
     ...
     */
    public static double[] convertTransverseMercatorToGeodetic(double easting,
            double northing) {
        ...
        // Origin
        double tmd = tmdo + (northing - tranMerc_false_northing)
                / tranMerc_scale_factor;
        ...
        double t11 = t
                * (5.e0 + 3.e0 * tan2 + eta - 4.e0 * Math.pow(eta, 2) - 9.e0
                        * tan2 * eta)
                / (24.e0 * sr * Math.pow(sn, 3) * Math
                        .pow(tranMerc_scale_factor, 4));
        double t12 = t
                * (61.e0 + 90.e0 * tan2 + 46.e0 * eta + 45.E0 * tan4 - 252.e0
                        * tan2 * eta - 3.e0 * eta2 + 100.e0 * eta3 - 66.e0
                        * tan2 * eta2 - 90.e0 * tan4 * eta + 88.e0 * eta4
                        + 225.e0 * tan4 * eta2 + 84.e0 * tan2 * eta3 - 192.e0
                        * tan2 * eta4)
                / (720.e0 * sr * Math.pow(sn, 5) * Math
                        .pow(tranMerc_scale_factor, 6));
        double t13 = t
                * (1385.e0 + 3633.e0 * tan2 + 4095.e0 * tan4 + 1575.e0 * Math
                        .pow(t, 6))
                / (40320.e0 * sr * Math.pow(sn, 7) * Math
                        .pow(tranMerc_scale_factor, 8));

        double latitude = ftphi - Math.pow(de, 2) * t10 + Math.pow(de, 4) * t11
                - Math.pow(de, 6) * t12 + Math.pow(de, 8) * t13;

        double t14 = 1.e0 / (sn * c * tranMerc_scale_factor);
        double t15 = (1.e0 + 2.e0 * tan2 + eta)
                / (6.e0 * Math.pow(sn, 3) * c * Math.pow(tranMerc_scale_factor,
                                                         3));
        double t16 = (5.e0 + 6.e0 * eta + 28.e0 * tan2 - 3.e0 * eta2 + 8.e0
                * tan2 * eta + 24.e0 * tan4 - 4.e0 * eta3 + 4.e0 * tan2 * eta2 + 24.e0
                * tan2 * eta3)
                / (120.e0 * Math.pow(sn, 5) * c * Math
                        .pow(tranMerc_scale_factor, 5));
        double t17 = (61.e0 + 662.e0 * tan2 + 1320.e0 * tan4 + 720.e0 * Math
                .pow(t, 6))
                / (5040.e0 * Math.pow(sn, 7) * c * Math
                        .pow(tranMerc_scale_factor, 7));

        // Difference in Longitude
        double dlam = de * t14 - Math.pow(de, 3) * t15 + Math.pow(de, 5) * t16
                - Math.pow(de, 7) * t17;
