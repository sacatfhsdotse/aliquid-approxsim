...
class UTMConversion {
    ...
    /*
     * This method receives the UTM zone override parameter as input, and sets 
    ...
    public static boolean setUTMParameters(long override) {
        ...
        // Override zone
            ...
            ProjectionErrorHandler.handleError(ProjectionErrorHandler.UTM_ZONE_OVERRIDE_ERROR);
            return false;
        }
        //
    ...
    /*
     * This method converts geodetic (latitude and longitude) coordinates to UTM projection 
     * (zone, hemisphere, easting and northing) coordinates according to the current ellipsoid 
     ...
     */    
    public static UTMCoordinate convertGeodeticToUTM (double longitude, double latitude) { 
        char hemisphere;
        ...
        double central_meridian = (temp_zone >= 31)? (6*temp_zone-183)*Math.PI/180.0 :
            (6*temp_zone+177)*Math.PI/180.0;
        ...
        TransverseMercatorProjection.setTransverseMercatorParameters(origin_latitude,central_meridian, 
                                                                     false_easting, false_northing, scale);
        double[] east_north = TransverseMercatorProjection.convertGeodeticToTransverseMercator(longitude, latitude);
        if (east_north == null) {
            return null;
        }
    ...
    /*
     * This method converts UTM projection (zone, hemisphere, easting 
    ...
    public static double[] convertUTMToGeodetic(UTMCoordinate utmc) {
        ...
        TransverseMercatorProjection.setTransverseMercatorParameters(origin_latitude, central_meridian, 
                                                                     false_easting, false_northing, scale);
        double[] lon_lat = TransverseMercatorProjection.convertTransverseMercatorToGeodetic(easting, northing);
