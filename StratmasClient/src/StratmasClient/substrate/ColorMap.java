...
public class ColorMap {
     ...
     */
    public static final String[] COLOR_MAPS = { "blue-green-yellow-red",
            "blue-green-yellow", "blue-yellow-orange-red",
            "white-lightgrey-cyan-black", "blue-white", "red-white",
            "black-white" };
    /**
     ...
     */
    public ColorMap(ColorChooser colorChooser, String scale, double minVal,
            double maxVal, String cmap) {
    ...
    private JPanel createColorMapPanel() {
        ...
        displayedValuesPanel.setBorder(BorderFactory.createEmptyBorder(1, 1, 1,
                                                                       1));
    ...
    private void selectColorMap(String cmapName) {
        if (cmapName.equals("blue-green-yellow-red")) {
            ...
            Color[] cmap = { Color.WHITE, Color.LIGHT_GRAY, Color.CYAN,
                    Color.BLACK };
            setColorMap(cmap);
    ...
    public void setDisplayedValues() {
        ...
        // linear scale
        if (isLinearScale()) {
            double increment = (getMaxValue() - getMinValue())
                    / (getNrOfDisplayedValues() - 1);
        ...
        // logarithmic scale
            ...
            // get the min value
            double minVal = Math.pow(10,
                                     Math.ceil(Math.log(getMaxValue())
                                             / Math.log(10))
                                             - getNrOfDisplayedValues() + 1);
            // all the values
            for (int i = 0; i < getNrOfDisplayedValues(); i++) {
    ...
    public Color getMappingColor(double value) {
        ...
        // linear scale
            ...
            return COLOR_TABLE[(int) Math.round((value - minValue)
                    / (maxValue - minValue) * (COLOR_SHADES - 1))];
        }
        // logarithmic scale
        else {
            double minVal = Math.log(displayedValues[0]) / Math.log(10);
            double maxVal = Math
                    .log(displayedValues[displayedValues.length - 1])
                    / Math.log(10);
            double val = (value <= 0) ? Math.log(displayedValues[0])
                    / Math.log(10) : Math.log(value) / Math.log(10);
            return COLOR_TABLE[(int) Math.round((val - minVal)
                    / (maxVal - minVal) * (COLOR_SHADES - 1))];
        }
    }
    ...
    public void updatePointedValue(Double value, int valPosition) {
        String valStr = (value == null) ? "" : convertToString(value
                .doubleValue());
