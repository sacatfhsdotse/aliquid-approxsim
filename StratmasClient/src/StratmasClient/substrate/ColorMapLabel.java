 ...
 */
public class ColorMapLabel extends JLabel implements MouseListener,
        MouseMotionListener {
    /**
	 * 
	 */
    ...
    /**
     * Updates the ColorChooser such that the label showing the value pointing by the mouse cursor is empty.
     */
    ...
    private double convertXToValue(double x) {
        ...
        // logarithmic scale
            ...
            pointedValue = Math.pow(10,
                                    x * (maxValue - minValue) / this.getWidth()
                                            + minValue);
        }
        return (x == 0) ? colorMap.getMinValue()
                : ((x == this.getWidth() - 1) ? maxVal : pointedValue);
    }
