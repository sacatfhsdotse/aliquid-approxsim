...
public class ActivityAdapter implements ApproxsimObjectAdapter,
     ...
     * @return true if the name is the renderSelectionName of the activity symbol, false otherwise.
     */
     ...
     * 
     * @param name the name to check.
     * @return true if the name is the renderSelectionName of the left arrow symbol, false otherwise.
     */
     ...
     * 
     * @param name the name to check.
     * @return true if the name is the renderSelectionName of the right arrow symbol, false otherwise.
     */
     ...
     */
    protected void updateActivityDisplayList(
            TimelineActivityPanel timelineActivityPanel, GL gl2) {
        GL2 gl = (GL2) gl2;
        activityDisplayList = (gl.glIsList(activityDisplayList)) ? activityDisplayList
                : gl.glGenLists(1);
        //
        ...
        int x1 = timelineActivityPanel
                .getXCoordinate(getLeftArrowPointedTime());
        // get x-coordinate of the render position of the right arrow
        int x2 = timelineActivityPanel
                .getXCoordinate(getRightArrowPointedTime());
        // get x-coordinate of the render position of the activity symbol
        int x = timelineActivityPanel
                .getXCoordinate((getLeftArrowPointedTime() + getRightArrowPointedTime()) / 2);
    ...
    /**
     * Updates (recreates) the display lists that draws all the symbols of the object this adapter represents.
     * 
    ...
    protected void updateSymbolDisplayLists(GLAutoDrawable gld) {
        ...
        for (int i = ACTIVITY_POS; i <= RIGHT_ARROW_POS; i++) {
            displayLists[i] = (gl.glIsList(displayLists[i])) ? displayLists[i]
                    : gl.glGenLists(1);
        }

        // create icons for the left and the right arrows
        Icon leftArrowIcon = new Icon(
                IconFactory.class.getResource("icons/left_arrow.png"));
        ...
        if (hasResource()) {
            ...
            leftArrowIcon = getArrowIcon((SymbolIDCode) mu
                                         ...
                                         leftArrowIcon.getImage(),
                                         leftArrowSymbols);
            rightArrowIcon = getArrowIcon((SymbolIDCode) mu
                                                  .getChild("symbolIDCode"),
                                          rightArrowIcon.getImage(),
                                          rightArrowSymbols);
        }

        // get textures from texture mapper
        int[] textures = new int[3];
        textures[0] = SymbolToTextureMapper.getTexture(getActivity().getIcon(),
                                                       gld);
        ...
        for (int i = ACTIVITY_POS; i <= RIGHT_ARROW_POS; i++) {
            ...
            if (i == ACTIVITY_POS) {
                createSymbol(gl, textures[i], getSymbolScale(),
                             getRenderSelectionName() + i, 1.0);
            } else {
                createSymbol(gl, textures[i], arrowScale,
                             getRenderSelectionName() + i, 1.0);
            }
            gl.glEndList();
        }
    ...
    /**
     * Used to create symbols used in the adapter. The symbols are : the activity symbol, the left arrow symbol and the right arrow symbol.
     * 
     ...
     */
    private void createSymbol(GL gl2, int texture, double symbolScale,
            int renderSelectionName, double alpha) {
        GL2 gl = (GL2) gl2;
        // Enable textures.
    ...
    /**
     * Draws the boundary box around the symbols representing the activity adapted by this adapter.
     * 
    ...
    public void drawBoundaryBox(GL gl2, int x1, int x2, int y) {
        ...
        gl.glVertex2d(x1 - getHorizontalSymbolSize() / 2, y
                - getVerticalSymbolSize() / 2);
        gl.glVertex2d(x1 - getHorizontalSymbolSize() / 2, y
                + getVerticalSymbolSize() / 2);
        gl.glVertex2d(x2 + getHorizontalSymbolSize() / 2, y
                + getVerticalSymbolSize() / 2);
        gl.glVertex2d(x2 + getHorizontalSymbolSize() / 2, y
                - getVerticalSymbolSize() / 2);
        gl.glEnd();
    ...
    /**
     * Returns the icons of the left and the right arrows. OBS. Potential memory leak - there is a risk that the keys and weak references in
     ...
     */
    public Icon getArrowIcon(SymbolIDCode code, Image src,
            Hashtable<String, WeakReference<Icon>> imageList) {
        String key = (code == null || code.valueToString().length() < 2) ? "-"
                : code.valueToString().substring(1, 2);
        ...
        if (icon != null) {
            ...
            Icon newIcon = new Icon(Toolkit.getDefaultToolkit()
    ...
    public boolean hasResource() {
        return activity.getParent().getParent().getType()
     ...
     * 
     * @param updated if true the symbol has been updated, if false the symbol has to be updated.
     */
    ...
    /**
     * Returns true if the activity this adapter represents is selected, otherwise false.
     */
     ...
     * 
     * @param selected if true the activity is selected, if false the symbol is not selected.
     */
    ...
    /**
     * Sets the symbol scale of this ActivityAdapter to the specified value (between 0.0 and 1.0 inclusive);
     * 
    ...
    protected void fireActivityRemoved() {
        ...
        for (int i = listeners.length - 1; i >= 0; i--) {
            if (listeners[i] instanceof ActivityAdapterListener) {
                ((ActivityAdapterListener) listeners[i])
                        .activityAdapterRemoved(this);
            }
        }
    }
    ...
    protected void fireActivitySelected(boolean selected) {
        ...
        for (int i = listeners.length - 1; i >= 0; i--) {
            if (listeners[i] instanceof ActivityAdapterListener) {
                ((ActivityAdapterListener) listeners[i])
                        .activityAdapterSelected(this, selected);
            }
        }
    }
}
