...
public class TimeInterval {
    ...
    /**
     * Return all times this interval contains such that the difference between two consecutive times is equal to the time step.
     */
    ...
    /**
     * Return all times in the subinterval of this interval such that the difference between two consecutive times is equal to the time
     * step.
     * 
    ...
    public long[] getAllTimes(long tstart, long tend) {
        ...
        if (tend < endTime) {
            long adj = ((tend - startTime) % timeStep == 0) ? (tend - startTime)
                    : ((tend - startTime) / timeStep) * timeStep;
            tend = startTime + adj;
        } else {
            tend = endTime;
        }
    ...
    /**
     * Sets the time step and adjusts the start and end times of the interval to the reference time.
     * 
    ...
    public boolean setTimeStep(long timeStep, long refTime) {
        ...
        if ((startTime - refTime) % this.timeStep != 0) {
            startTime = refTime + ((startTime - refTime) / this.timeStep + 1)
                    * this.timeStep;
        }
        if ((endTime - refTime) % this.timeStep != 0) {
            endTime = refTime + (endTime - refTime) / this.timeStep
                    * this.timeStep;
        }
     ...
     * 
     * @param indicator indicates if the interval will be increased from the left or from the right.
     */
     ...
     * 
     * @param indicator indicates if the interval will be decreased from the left or from the right.
     */
    ...
    public boolean intersects(TimeInterval interval) {
        return (!(getEndTime() < interval.getStartTime() || getStartTime() > interval
                .getEndTime()));
    }
    ...
    public boolean contains(TimeInterval interval) {
        return ((getStartTime() <= interval.getStartTime()) && (getEndTime() >= interval
                .getEndTime()));
    }

    /**
     * Adds new time interval to this one. Obs. This method should be called only if the two intervals intersect.
     */
    ...
    public TimeInterval[] remove(TimeInterval interval) {
        ...
        TimeInterval t1 = (tmpStart >= startTime) ? new TimeInterval(timeStep,
                startTime, tmpStart) : null;
        long tmpEnd = interval.getEndTime() + interval.getTimeStep();
        TimeInterval t2 = (tmpEnd <= endTime) ? new TimeInterval(timeStep,
                tmpEnd, endTime) : null;
        //
        if (t1 != null && t2 != null) {
