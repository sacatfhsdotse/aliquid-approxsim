...
public class TimelinePanel extends JPanel {
    ...
    public TimelinePanel(Timeline timeline) {
        ...
        northPanel.add(displayControl.getTimelineTools(this),
                       BorderLayout.NORTH);
        northPanel.add(timelineActivityPanel, BorderLayout.CENTER);
        northPanel.add(displayControl.getTimelineSliderPanel(this),
                       BorderLayout.SOUTH);
    ...
    /**
     * Converts the time counted according to the actual time unit to the milliseconds counted relative to the simulation start time.
     * 
    ...
    /**
     * Converts milliseconds to the actual time unit. The milliseconds are counted relative to the simulation start time.
     * 
     ...
     * @param format format of the output <code>String</code>. The format can be compact (<code>TimelinePanel.COMPACT</code>) and long (
     *            <code>TimelinePanel.LONG</code>).
    ...
    public static String millisecondsToString(long time, int format) {
        ...
        if (format == TimelineConstants.COMPACT) {
            ...
            return new String(days_str + ":" + hours_str + ":" + minutes_str
                    + ":" + seconds_str);
        }
        // long format
        else {
            String seconds_str = (seconds == 0) ? "" : ((seconds > 1) ? seconds
                    + " secs " : seconds + " sec ");
            String minutes_str = (minutes == 0) ? "" : ((minutes > 1) ? minutes
                    + " mins " : minutes + " min ");
            String hours_str = (hours == 0) ? "" : ((hours > 1) ? hours
                    + " hours " : hours + " hour ");
            String days_str = (days == 0) ? "" : ((days > 1) ? days + " days "
                    : days + " day ");
            return new String(days_str + "" + hours_str + "" + minutes_str + ""
                    + seconds_str);
        }
    ...
    public void zoomInScale() {
        ...
        if (interval > 100
            ...
            setSmallerTimeUnit();
            interval = 100;
            long tu = (getCurrentTimeUnit() == TimelineConstants.HOUR) ? 24
                    : 60;
    ...
    public void zoomOutScale() {
        ...
        long interval = tend - tstart;
        if (interval <= 10
                || (getCurrentTimeUnit() == TimelineConstants.DAY && interval < maxTimeBound)) {
