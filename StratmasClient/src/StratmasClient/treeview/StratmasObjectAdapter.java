                                              ...
                                              StratmasClient.filter.StratmasObjectAdapter
{
    ...
    /**
     * Adds the provided adapter as a child to this, if it passes
     * filter. Returns true if the child was added.
     *
     * @param child the child to add.
    ...
    /**
     * Returns the root of the treemodel (this will always be this
     * since the TreeModel is, so to speek, a facet of this object.)
     */
    ...
    /**
     * Returns the index of the specified subnode, or -1 if no such
     * node is a child.
     *
    ...
    /**
     * Returns the TreeNode holding this object or null if
    ...
    /**
     * Removes a listener for the TreeModelEvent, previously added
    ...
    /**
     * Creates StratmasObjectAdapters suitable for the given
     * object. This is the approved method of getting
     * StratmasObjectAdapters...
     *
    ...
    /**
     * Disposes this adapter, making it unfit for further use and
    ...
    /**
     * Adds an InvisibleListListener. The listener does not listen to
     * the adapter directly but to a StramtasList that is a child of
     * the object the adapter adapts. Since this list is not adapted
     * by any StratmasObjectAdapter it must be listened to explicitly
     * in order to be able to forward add events for the list to the
     * adapter that needs to add a new child adapter for the added
     * object.
     *
     * @param listener the listener to add.
     */
...
/**
 * An InvisibleListListener listens to lists that is not visible in a
 * TreeView (and thus not adapted by any StratmasObjectAdapter) due to
 * the current filter. When elements are added to the list the
 * listener forwards the add (i.e. adds the object) to the actual
 * adapter.
 *
 * @version 1, $Date: 2006/08/31 14:45:12 $
 * @author  Per Alexius
