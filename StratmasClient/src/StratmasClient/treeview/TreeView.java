...
public class TreeView extends JTree implements DragGestureListener,
    ...
    /**
     * A reference to the current Window this JTree resides in, or null if none.
     */
    ...
    protected void initZoom() {
        ...
        getActionMap()
                .put("ZoomIn",
                     new AbstractAction("Zoom In", new ImageIcon(TreeView.class
                             .getResource("images/zoom_in.png"))) {
                         /**
							 * 
							 */
        ...
        getActionMap()
                .put("ZoomOut",
                     new AbstractAction("Zoom Out", new ImageIcon(
                             TreeView.class.getResource("images/zoom_out.png"))) {
    ...
    /**
     * Helper function, checks if object in vector are possible to add to provided object.
     * 
     * @param candidates
     * @param target
     */
        ...
        if (target.isLeaf()) {
            ...
            for (Enumeration e = candidates.elements(); e.hasMoreElements();) {
                ...
                if (target.getChild(candidate.getIdentifier()) != null
                        || target.getType()
                                .getSubElement(candidate.getIdentifier()) == null) {
                    status = false;
                    break;
                }
            }
    ...
    protected void showPopup(int x, int y, ApproxsimObjectAdapter objectAdapter) {
        ...
        popup.add(new AbstractAction("View branch in separate tree") {
            ...
            public void actionPerformed(ActionEvent e) {
                final TreeViewFrame frame = TreeView.getDefaultFrame(sObj);
                frame.setEditable(isEditable());
                if (self.getTopLevelAncestor() instanceof ClientMainFrame) {
                    ((ClientMainFrame) self.getTopLevelAncestor())
                            .tabFrame(frame);
                } else {
        ...
        popup.add(new AbstractAction("View only " + sObj.getType().getName()
                + "(s)") {
            /**
				 * 
				 */
    ...
    public void dragGestureRecognized(DragGestureEvent dge) {
        ...
        try {
            ...
            Debug.err
                    .println("InvalidDnDOperationException caught in TreeView dragGestuerRecognized: "
                            + e);
    ...
    /**
     * Symmetrically scales the prefered width and height of icons used in the TreeCellRenderes. Note that if scale > 1 (<1) the icon will
     * always increase (decrease) with at least one pixel.
     * 
    ...
    /**
     * Sets the prefered width and height of icons used in the TreeCellRenderes if width <= 0 || height <= 0 nothing will happen.
     * 
    ...
    /**
     * Gets the prefered width and height of icons used in the TreeCellRenderes
     */
...
//        setModel(null);
    ...
    /**
     * Invoked when a window is changed from a normal to a minimized state. For many platforms, a minimized window is displayed as the icon
    ...
    /**
     * Invoked when the Window is set to be the active Window. Only a Frame or a Dialog can be the active Window. The native windowing
     * system may denote the active Window or its children with special decorations, such as a highlighted title bar. The active Window is
     * always either the focused Window, or the first Frame or Dialog that is an owner of the focused Window.
     */
    ...
    /**
     * Invoked when a Window is no longer the active Window. Only a Frame or a Dialog can be the active Window. The native windowing system
     * may denote the active Window or its children with special decorations, such as a highlighted title bar. The active Window is always
     * either the focused Window, or the first Frame or Dialog that is an owner of the focused Window.
     */
