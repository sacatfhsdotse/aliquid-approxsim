/*
 * A SymbolViewer that uses PNG images originally supplied by the Norwegian Defense and
 * tweaked into PNGs by me. Not bound to PNG format technically, uses javax.imageio.
*/
...
//                                 }
                ...
                static protected String getAffilSuffix(SymbolID id) {
                        return SymbolID.AffiliationField.getIconAlias(id.getFieldValue(SymbolID.F_AFFILIATION)).toLowerCase() +
                        suffix;
                }
...
//                 }
        ...
        static private class IconLoader extends LayerLoader {
                ...
                public String getFilename(SymbolID sid) {
                        ...
                        if(address.startsWith("1.X.1"))
                                ...
                                filename.append("1x5");
                        else if(address.startsWith("1.X.6"))
                                filename.append("1x6");
                        ...
                        String affil = SymbolID.AffiliationField.getIconAlias(sid.getFieldValue(SymbolID.F_AFFILIATION));
        ...
        static private class EchelonLoader extends LayerLoader {
                ...
                public String getFilename(SymbolID sid) {
                        ...
                        if(needSize.indexOf(ech.charAt(0)) >= 0) {
                                ...
                                if(si >= 0) {
                                        si++;
                                        StringBuffer ind = new StringBuffer("indicators/" + getSize() + "/");
                                        if(si < 10)
                                                ind.append("0");
                                        ind.append(si);
                                        String a = sid.getFieldValue(SymbolID.F_AFFILIATION);
                                        ind.append(SymbolID.AffiliationField.getIconAlias(a).toLowerCase());
                                        ind.append(suffix);
        ...
        static private class FeintDummyLoader extends LayerLoader {
                ...
                public String getFilename(SymbolID sid) {
                        if(sid.isFeintDummy()) {
                                StringBuffer fn = new StringBuffer("indicators/" + getSize() + "/Fd");
                                String affil = getAffilSuffix(sid);
        ...
        static private class HQLoader extends LayerLoader {
                ...
                public String getFilename(SymbolID sid) {
                        if(sid.isHeadquarters()) {
                                StringBuffer hq = new StringBuffer("indicators/" + getSize() + "/Hq_");
                                hq.append(sid.getFieldValue(SymbolID.F_AFFILIATION).toLowerCase());
                                hq.append(suffix);
                                return hq.toString();
                        }
                        return null;
        ...
        static private class MobilityLoader extends LayerLoader {
                ...
                public String getFilename(SymbolID sid) {
                        ...
                        if(m > 0 && m < 10)
                                mob = "0" + m;
                        else if(m >= 10 && m <= SymbolID.SymbolModifierField.EQUIP_AMPHIBIOUS)
                                mob = "" + m;
                        if(mob.length() > 0)
        ...
        static private class AffiliationModifierLoader extends LayerLoader {
                ...
                public String getFilename(SymbolID sid) {
                        String prefix = null;
                        final char affil = sid.getFieldValue(SymbolID.F_AFFILIATION).charAt(0);
                        if(affil == 'A')
                                prefix = "Af_";
                        else if(affil == 'J')
                                prefix ="Jkr_";
        ...
        public ImageSymbolLoader(String rootDir, int cacheSize) {
                ...
                canvasBorder = new Rectangle(0, 0, canvas.getWidth(), canvas.getHeight());
        ...
        public Image load(String symbolID) {
                ...
                for(int i = 0; i < layer.length; i++) {
                        ...
                        if(img != null) {
                                if(frame == null) {
                                        frame = new Rectangle((canvas.getWidth() - img.getWidth(null)) / 2,
                                ...
                                if(i == 0)
                                        ...
                                        int lx = frame.x + (int) (frame.width  * layer[i].getOffsetX()),
                                            ly = frame.y + (int) (frame.height * layer[i].getOffsetY());

                                        canvas2D.drawImage(img, lx, ly, null);
                                        bound.add(lx, ly);
                                        bound.add(lx + ((BufferedImage) img).getWidth(), ly + ((BufferedImage) img).getHeight());
                                }
                        }
                }
                if(bound != null) {
                        ...
                        BufferedImage img = canvas.getSubimage(bound.x, bound.y, bound.width, 
                                                               bound.height);
