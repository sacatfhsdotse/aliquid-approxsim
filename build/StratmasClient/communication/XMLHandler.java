...
/**
 * This is the class where all XML from the server is parsed and
 * distributed to its proper place.
 *
...
public class XMLHandler implements Runnable {
      ...
      * @param grammar The name of the schema file to use for
      * validation.
      */
     public XMLHandler(Client client, String grammar) {
          ...
          if (!(grammar == null)) {
               ...
               try {
                   preparser.setEntityResolver(new LSJarXSDResolver(preparser.getEntityResolver()));
                   XMLInputSource source = 
                       preparser.getEntityResolver().resolveEntity(new XMLResourceIdentifierImpl(null, grammar, null, null));
               ...
               parserConfiguration.setEntityResolver(new LSJarXSDResolver(parserConfiguration.getEntityResolver()));
               ...
               parserConfiguration.setFeature(SCHEMA_VALIDATION_FEATURE_ID, validate);
               parserConfiguration.setFeature(SCHEMA_FULL_CHECKING_FEATURE_ID, validate);
    ...
    /**
      * Creates an XMLHandler operating independently of any Client,
      * initializes a default DOMParser.
      */
    public XMLHandler() 
    {
    ...
    public SubscriptionHandler getSubscriptionHandler()
    {
     ...
     /**
      * Checks if the threshold for the number of incomming messages
      * is reached.
      *
      ...
      * @param msg The StratmasMessage that generated the response now
      * to be handled.
      */
...
//               mClient.updateStatus(new Hashtable(), "GetGridMessage");
          ...
          for (Node child = nn.getFirstChild(); child != null; child = child.getNextSibling()) {
               if (child.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (elem.getTagName().equals("pv")) {
                         Element range = XMLHandler.getFirstChildByTag(elem, "range");
                         pvs.add(new ProcessVariableDescription(XMLHandler.getString(elem  , "name"    ),
                                                                XMLHandler.getString(elem  , "category"),
                                                                XMLHandler.getBoolean(elem , "factions"),
     ...
     private Timestamp handleSubscribedDataMsg(Element n) {
          Timestamp t = new Timestamp(getLong(getFirstChildByTag(n, "simulationTime"), "value"));
          Debug.err.println("Current time: " + t + ". Now handling subscriptions...");
          for (Node child = n.getFirstChild(); child != null; child = child.getNextSibling()) {
               if (child.getNodeType() == Node.ELEMENT_NODE && child.getNodeName().equals("subscribedData")) {
     ...
     private Timestamp handleUpdateMsg(Element n) {
          String tsStr = getString(getFirstChildByTag(n, "simulationTime"), "value");
          Timestamp t;
          try {
               t = new Timestamp(Timestamp.parseDateTime(tsStr).getTime());
          } catch (java.text.ParseException e) {
               throw new AssertionError("Failed to extract a dateTime from '" + tsStr + "'.");
          }
...
/// Helpers
     ...
     public static Type getType(Element element) {
          TypeInfo typeInfo = ((org.apache.xerces.dom.ElementImpl)element).getSchemaTypeInfo();
          Type ret = TypeFactory.getType(typeInfo.getTypeName(), typeInfo.getTypeNamespace());
          if (ret == null) {
               ret = TypeFactory.getType(removeNamespace(element.getAttribute("xsi:type")));
               if (ret == null) {
                    throw new AssertionError("getType() failed for element " + element.getTagName() + 
     ...
     /**
      * Gets the type of StratmasObject represented by the provided
      * dom element. First check the xsi:type attribute and if it
      * doesn't exist use the 1.5 specific method getSchemaTypeInfo
      * method. Since Xerces supports this method it should be
      * possible to call it even though 1.4.2 is used.
      *
      * @param element The dom element to get the type for.
      * @return The type of StratmasObject the element represents.
      */
               ...
               try {
                    ...
                    if (ret == null) {
                         throw new AssertionError("Can't create Type object for '" + typeInfo.getTypeName() +
                                                  "' in namespace '" + typeInfo.getTypeNamespace() + "'");
                    }
     ...
     public static String getString(Node n, String tag) {
          ...
          if (elem == null) {
               System.err.println("No element with tag '" + tag + "' in Node with name " + n.getNodeName());
               return "";
          }
          else {
     ...
     /**
      * Removes the namespace from the provided String, that is,
      * removes all characters from the beginning of the String to
      * (and including) the last occurence of the ':' character.
     ...
     public static void printNode(Node n) {
          try {
               ...
               System.err.println(domImplementationLS.createLSSerializer().writeToString(n));
...
/**
 * Simple class for grouping the message and its response together
 * before enqueuing them in the XMLHandlers queue.
 *
 * @version 1, $Date: 2006/09/11 09:33:42 $
 * @author  Per Alexius
 */
...
/**
 * Simple exception used to signify errors between the different
 * handle-levels
 */
