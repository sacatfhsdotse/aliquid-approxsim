/*
 * A tree that can hold the symbol hierarchy. Meant to be initialized from a copy-and-pasted
 * textfile holding lines looking like this (from the beginning):
 * 
 * 1.X S * U * -- -- -- * * * UNKNOWN/UNKNOWN
 * 1.X.1 S * P * -- -- -- ** ** * SPACE TRACK
 * 1.X.1.1 S * P * S- -- -- ** ** * SATELLITE
 * 1.X.1.2 S * P * V- -- -- ** ** * CREWED SPACE VEHICLE
 * 1.X.1.3 S * P * T- -- -- ** ** * SPACE STATION
 * 
 * And so on. This is not very elegant or "industrial strength", but it still beats typing
 * them in all by hand, and is a pretty quick way to extract the text from the PDFs of the
 * MIL-STD-2525B standard (TABLE A-III and friends).
 * 
 * The fields we care about of the above, are:
 * address
 * 	The first field, the dot-separated string of symbols. Locates each node
 * 	in the hierarchy, and also defines the hierarchy itself.
 * symbol id
 * 	Joined together from the second, fourth, and fifth through seventh fields,
 * 	so for the SPACE STATION it is "SPT-----", formatted just like that.
 * function name
 * 	Joined together from all words after the first 11 fields, with spaces in
 * 	between.
*/
		...
		try {
			for(String line; (line = r.readLine()) != null;) {
				...
				if(p != null) {
					...
					FunctionIDNode node = new FunctionIDNode(adr, symid.toString(), func.toString());
					...
					System.out.println("FunctionIDTree: Bad hierarchy parent '" + parent + "' at line " + r.getLineNumber());
					return;
				}
			}
		}
