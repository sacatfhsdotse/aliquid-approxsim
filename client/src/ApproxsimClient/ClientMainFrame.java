...
public class ClientMainFrame extends JFrame {
     ...
     */
    public static void activateProgressBar(final boolean activate,
            final String info) {
        if (progressBar != null) {
    ...
    protected void createDnDHandler() {
        getTabPane().setDropTarget(new DropTarget(getTabPane(),
                                           new DropTargetAdapter() {
                                               ...
                                               public void dragOver(
                                                       DropTargetDragEvent dtde) {
                                                   tabFocusDelayTimer.cancel();
                                                   final int index = getTabPane()
                                                           .indexAtLocation((int) dtde
                                                                                    .getLocation()
                                                                                    .getX(),
                                                                            (int) dtde
                                                                                    .getLocation()
        ...
        getTabPane().addMouseListener(new MouseInputAdapter() {
            public void mousePressed(MouseEvent e) {
                if (e.getButton() == MouseEvent.BUTTON3) {
                    final int index = getTabPane().indexAtLocation(e.getX(),
                                                                   e.getY());
                    if (index != -1) {
                        ...
                        menu.add(new AbstractAction("Evict") {
                            ...
                            public void actionPerformed(ActionEvent ev) {
                                Component component = getTabPane()
        ...
        if (Debug.isInDebugMode()) {
            ...
            debugMenu.addMenuListener(new MenuListener() {
                ...
                public void menuSelected(MenuEvent e) {
                    JMenu menu = (JMenu) e.getSource();
                    menu.removeAll();
                    for (Iterator<Action> it = Debug.getDebugActions()
                            .iterator(); it.hasNext(); menu.add((Action) it
                            .next()));
                }
            });
    ...
    public void updateToolMenu(JMenu menu) {
        ...
        Action importResourceAction = new AbstractAction(
                "Import from IconFactory2") {
            /**
			 * 
			 */
        ...
        importResourceAction.setEnabled(getClient().getRootObject()
                .getChildCount() > 0);
                        ...
                        public void run() {
                            ...
                            if (list != null) {
                                final TypeFilter filter = new TypeFilter(
                                        TypeFactory.getType("Order"));
                                final JFrame frame = OrderImportTreeView
                                        .getDefaultFrame(list, filter);
        ...
        importOrderLib
                .setEnabled(getClient().getRootObject().getChildCount() > 0);
                    ...
                    Thread thread = new Thread() {
                        public void run() {
                            ...
                            if (list != null) {
                                ...
                                final JFrame frame = HierarchyImportTreeView
                                        .getDefaultFrame(list, filter);
                                frame.setTitle(filename);
                                frame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
                                SwingUtilities.invokeLater(new Runnable() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (getClient().getRootObject().children().hasMoreElements()) {
                    ...
                    if (filename != null) {
                        ...
                        Thread thread = new Thread() {
                            public void run() {
                                if (ext.equalsIgnoreCase(FileExtensionFilter
                                        .getExtension(file))) {
    ...
    public void updateViewMenu(JMenu menu) {
        ...
        for (int i = 0; i < frames.length; i++) {
            ...
            final WeakReference<Frame> frameReference = new WeakReference<Frame>(
                    frame);
            JCheckBoxMenuItem showMenuItem = new JCheckBoxMenuItem(
                    new AbstractAction(frame.getTitle()) {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            JCheckBoxMenuItem box = (JCheckBoxMenuItem) e
                                    .getSource();
            ...
            if (frame instanceof JFrame) {
                JCheckBoxMenuItem tabMenuItem = new JCheckBoxMenuItem(
                        new AbstractAction(frame.getTitle()) {
                            /**
							 * 
							 */
        ...
        for (int i = 0; i < this.tabPane.getTabCount(); i++) {
            ...
            JCheckBoxMenuItem tabMenuItem = new JCheckBoxMenuItem(
                    new AbstractAction(this.tabPane.getTitleAt(i)) {
                        /**
						 * 
						 */
...
//         }
    ...
    public void addTab(String title, ImageIcon icon, Component component) {
        ...
        this.tabPane.addTab(title, icon, component);
    ...
    private JFrame frameTabImpl(Component component) {
        ...
        if (frame == null) {
            String title = this.tabPane.getTitleAt(this.tabPane
                    .indexOfComponent(component));
            ImageIcon icon = (ImageIcon) this.tabPane.getIconAt(this.tabPane
                    .indexOfComponent(component));
            frame = new JFrame();
...
//             });
     ...
     */
    public void dispose() {
        if (JOptionPane.showConfirmDialog(this, "Really quit?", "Exiting...",
...
/**
 * This dialog is used to import military units from IconFactory2 and initialize these units with default values from the table source file.
 */
class ImportSourceDialog extends JDialog {
    ...
    public ImportSourceDialog(ClientMainFrame clientMainFrame) {
        ...
        importFromIF2Panel.setBorder(BorderFactory
    ...
    private JPanel createSourceFilePanel() {
        ...
        fileButton.addActionListener(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                String filename = Client
                        .getFileNameFromDialog(".xml", JFileChooser.OPEN_DIALOG);
                if (filename != null) {
        ...
        filePanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
                .createTitledBorder("Select file"), BorderFactory
                .createEmptyBorder(5, 5, 5, 5)));
        return filePanel;
    }

    /**
     * Creates the panel which shows the name of the file with the default values for different military units.
     */
    private JPanel createTableDefaultPanel() {
        ...
        tableButton.addActionListener(new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                String filename = Client
                        .getFileNameFromDialog(".csv", JFileChooser.OPEN_DIALOG);
                if (filename != null) {
        ...
        tablePanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory
                .createTitledBorder("Select table"), BorderFactory
                .createEmptyBorder(5, 5, 5, 5)));
        return tablePanel;
    }
    ...
    private JComboBox createFactionList() {
        ...
        factionBox
                .setBorder(BorderFactory.createCompoundBorder(BorderFactory
                                                                      .createTitledBorder("Select affiliation faction"),
                                                              BorderFactory
                                                                      .createEmptyBorder(5,
                                                                                         5,
                                                                                         5,
    ...
    /**
     * Imports the military units from IconFactory2 and sets the default values according to the table described in the table source file.
     */
    private void importUnitsAndSetDefaults(String filename) {
        ...
        if (tableFilename.length() > 0) {
            try {
                UnitImportFilter unitFilter = new UnitImportFilter(
                        new FileInputStream(tableFilename));
                applyFilterToSubtree(so, unitFilter, filter);
                actualSymbolIDCodeMappingTable = tableFilename;
            } catch (java.io.IOException exc) {} catch (java.text.ParseException exc) {}
        }
        // the subtree is created
        if (so != null) {
            final JFrame frame = HierarchyImportTreeView
                    .getDefaultFrame(so, filter);
            frame.setTitle(filename);
            frame.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
