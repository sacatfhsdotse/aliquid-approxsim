...
//         gl.glClearColor(r, g, b, 1.0f);
    ...
    private int scaleXValue(long t) {
        return (int) (xmin + (xmax - xmin)
                * ((double) (t - graph.getStartTime()))
    ...
    private Long scaleYValue(double yval) {
        ...
        if (graph.getActualScale().equals("Linear Scale")) {
            ...
            return new Long((long) (ymin + (ymax - ymin)
                    * (yval - graph.getLowerYBound()) / yDiff));
        }
        // logarithmic scale
        else {
            ...
            if (graph.getLowerYBound() == 0) {
                ...
                if (yval >= graph.getSecondLowerYBound()) {
                    ...
                    double subInterval = (ymax - ymin)
                            / (logPVMax - logPVMin + 1);
                    double lg = subInterval + ymin
                            + (ymax - ymin - subInterval) * (logPV - logPVMin)
                            / (logPVMax - logPVMin);
                ...
                // the value is smaller then the second lowest displayed value on the y-axis but non-negative
                    ...
                    double subInterval = (ymax - ymin)
                            / (logPVMax - logPVMin + 2);
                    double lg = ymin + subInterval * (logPV - logPVMin)
                            / (logPVMax - logPVMin);
                    return new Long((long) lg);
            ...
            // the lower bound is larger then zero
                ...
                if (yval > 0) {
                    ...
                    double logPVMax = graph.log10(graph.getUpperYBound());
                    double logPV = graph.log10(yval);
                    double lg = ymin + (ymax - ymin) * (logPV - logPVMin)
                            / (logPVMax - logPVMin);
                    return new Long((long) lg);
                }
    ...
    private void setUpVerticalLines(GL gl2) {
        GL2 gl = (GL2) gl2;
        verticalLinesDisplayList = (gl.glIsList(verticalLinesDisplayList)) ? verticalLinesDisplayList
                : gl.glGenLists(1);
...
//        gl.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    private double[] getHorizontalLineValues() {
        ...
        if (graph.isLinearScale()) {
            double partDiff = (graph.getUpperYBound() - graph.getLowerYBound())
                    / (yValues.length - 1);
        ...
        // logarithmic scale
            ...
            if (graph.getLowerYBound() > 0
                ...
                for (int i = 0; i < ProcessVariableXYGraph.MIDDLE_LOG_VALUES.length; i++) {
                    yValues[i + 1] = graph.getLowerYBound()
    ...
    private void drawCurrentValues(GL gl2) {
        ...
        for (int i = 0; i < factions.length; i++) {
            ...
            for (int j = 0; j < currentValues.times.size(); j++) {
                // get time unit
                long time = graph
                        .getRelativeGraphTime(((Long) currentValues.times
                                .get(j)).longValue());
                // get value
                Number val = (Number) ((Hashtable) currentValues.values.get(j))
                        .get(factions[i]);
                Long scaledValue = (val != null) ? scaleYValue(val
                        .doubleValue()) : null;
                if (scaledValue != null) {
                    // scale the time
    ...
    private void drawPreviousValues(GL gl2) {
        ...
        for (int i = 0; i < factions.length; i++) {
            ...
            for (int j = 0; j < previousValues.times.size(); j++) {
                // get time unit
                long time = graph
                        .getRelativeGraphTime(((Long) previousValues.times
                                .get(j)).longValue());
                // get value
