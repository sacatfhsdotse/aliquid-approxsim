...
public class SubstrateXMLWriter {
     ...
     */
    public static void convertToXML(BufferedWriter writer, Vector values)
            throws IOException {
    ...
    private static void convertToXML(BufferedWriter writer,
            ProcessVariableInitialValues values) throws IOException {
        writeLine(writer,
                  "<pviv xsi:type=\"sp:ProcessVariableInitialValuesForFile\">");
        // process variable
     ...
     */
    private static void convertToXML(BufferedWriter writer,
            ProcessVariableDescription pv) throws IOException {
     ...
     */
    private static void convertToXML(BufferedWriter writer, ShapeValuePair svp)
            throws IOException {
        if (svp.isEsri()) {
            writeLine(writer, "<regions xsi:type=\"sp:ESRIRegion\">");
            writeLine(writer, "<value>" + svp.getValue() + "</value>");
            writeLine(writer, "<reference>");
            writeLine(writer, "<name>" + svp.getShape().getIdentifier()
                    + "</name>");
     ...
     */
    private static void convertToXML(BufferedWriter writer, Circle circle)
            throws IOException {
        writeLine(writer, "<shape xsi:type=\"sp:Circle\" identifier=\""
                + circle.getIdentifier() + "\">");
        writeLine(writer, "<radius><value>" + circle.getRadius()
                + "</value></radius>");
        writeLine(writer, "<center><lat>" + circle.getCenter().getLat()
                + "</lat><lon>" + circle.getCenter().getLon()
                + "</lon></center>");
     ...
     */
    private static void convertToXML(BufferedWriter writer, Polygon polygon)
            throws IOException {
        writeLine(writer, "<shape xsi:type=\"sp:Polygon\" identifier=\""
                + polygon.getIdentifier() + "\">");
        for (Enumeration e = polygon.getCurves(); e.hasMoreElements();) {
            Line line = (Line) e.nextElement();
            writeLine(writer, "<curves xsi:type=\"sp:Line\" identifier=\""
                    + line.getIdentifier() + "\">");
            writeLine(writer, "<p1><lat>" + line.getStartPoint().getLat()
                    + "</lat><lon>" + line.getStartPoint().getLon()
                    + "</lon></p1>");
            writeLine(writer, "<p2><lat>" + line.getEndPoint().getLat()
                    + "</lat><lon>" + line.getEndPoint().getLon()
                    + "</lon></p2>");
     ...
     * @param values the values obtained from the editor.
     */
    public static void convertToXMLForServer(BufferedWriter writer,
            Vector values) throws IOException {
        for (int i = 0; i < values.size(); i++) {
            ProcessVariableInitialValues pviv = (ProcessVariableInitialValues) values
                    .get(i);
            writeLine(writer,
                      "<pvinitvalues xsi:type=\"sp:ProcessVariableInitialValuesForServer\">");
            // process variable
